...
public class VDoctorNotes extends DoctorNotes
		implements FormPanel, ActionListener, TableModelListener, VetoableChangeListener
{
	...
	private void jbInit()
	{
		...
		orderLabel.setText("Doctor  Notes");
		...
		infoPanel.setPreferredSize(new Dimension(1200, 250));
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		{
			NewNote newNote = new NewNote((Frame) null, ZEnv.getHms_treatment_doc_ID());
			AEnv.showCenterScreen(newNote);
		...
		public void mouseClicked(MouseEvent me)
		{
			...
			if (me.getClickCount() == 2)
			{
				try
				{
					...
					m_note_ID = idColumn.getRecord_ID();
					EditNote editNote = new EditNote((Frame) null, m_note_ID);
					AEnv.showCenterScreen(editNote);
				} catch (Exception e)
				{
					e.printStackTrace();
				}
	...
	private boolean editable(int recordID)
	{
		...
		// if
		// (DateUtil.newDate().before(DateUtil.getDateTime(note.getCreated())))
		// return true;
		// else
		return false;
	...
	public void loadBPartner()
	{
		...
		Vector<Vector<Object>> data = getNotesData(whereClause);
