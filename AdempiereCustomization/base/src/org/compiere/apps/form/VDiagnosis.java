...
import zenith.model.X_hms_pddiagnosis;
import zenith.util.DateUtil;

import org.compiere.grid.ed.EditDiagnosis;
import org.compiere.grid.ed.EditDrug;
import org.compiere.grid.ed.EditVitals;
import org.compiere.grid.ed.NewDiagnosis;
import org.compiere.grid.ed.NewDrug;

public class VDiagnosis extends Diagnosis
		implements FormPanel, ActionListener, TableModelListener, VetoableChangeListener
{
	...
	public void dynInit() throws Exception
	{
		...
		// Translation
		statusBar.setStatusLine("Patient Diagnosis");
	...
	private void jbInit()
	{
		...
		newButton.setText("New Patient Diagnosis");
		newButton.setPreferredSize(new Dimension(300, 30));
		newButton.setFont(new Font("Arial", Font.BOLD, 20));
		newButton.addActionListener(this);

		orderLabel.setText("Patient Diagnosis");
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		{
			NewDiagnosis newDiagnosis = new NewDiagnosis((Frame) null, ZEnv.getHms_treatment_doc_ID());
			AEnv.showCenterScreen(newDiagnosis);
		...
		public void mouseClicked(MouseEvent me)
		{
			...
			if (me.getClickCount() == 2)
			{
				try
				{
					...
					m_Diag_ID = idColumn.getRecord_ID();
					if (editable(m_Diag_ID))
					{
						EditDiagnosis editDiag = new EditDiagnosis((Frame) null, m_Diag_ID);
						AEnv.showCenterScreen(editDiag);
					}
				} catch (Exception e)
				{
					e.printStackTrace();
	...
	{
		X_hms_pddiagnosis diag = new X_hms_pddiagnosis(Env.getCtx(), recordID, null);
		if (DateUtil.newDate().before(DateUtil.getDateTime(diag.getCreated())))
			return true;
		return false;
	}
	...
	public void loadBPartner()
	{
		...
		Vector<Vector<Object>> data = getDiagnosisData(whereClause);
		...
		// set the preferred width of diagnosis name column.. mathew 06.11.2017
		try
		{
			int column = 3;
