...
import org.compiere.grid.VTableBackgrounds;
...
public final class VCellRenderer extends DefaultTableCellRenderer
{
			...
			int row, int col)
	{
		...
		//
		c.setBackground(bg);
		VTableBackgrounds vtbs = setTableAndRow(table, row);
		if (row == vtbs.getRowNum())
		{
			c.setForeground(vtbs.getColor());
			c.setFont(vtbs.getFont());
		} else
		{
			c.setForeground(fg);
		}
	 ...
	 * other style constants (except PLAIN) for mixed styles. e.g 1+2=3 for Bold
	 * and Italic.
	 * 
	 * For consistency maintain 'Dialog' as the font and '14' as the size.
	 * 
	 * @param table
	 *            the current JTable
	 * @param row
	...
	protected void setValue(Object value)
	{
		...
		try
		{
			...
			// Row ID
				...
				retValue = "";
			// Lookup
			else if (m_lookup != null && (DisplayType.isLookup(m_displayType) || m_displayType == DisplayType.Location
					|| m_displayType == DisplayType.Account || m_displayType == DisplayType.Locator
	...
	{
		return "VCellRenderer[" + m_columnName + ",DisplayType=" + m_displayType + " - " + m_lookup + "]";
	} // toString
