...
public class VDate extends JComponent implements VEditor, ActionListener, KeyListener, FocusListener
{
	 ...
	 *            title
	 */
	public VDate(String columnName, boolean mandatory, boolean isReadOnly, boolean isUpdateable, int displayType,
			String title)
	{
		super();
	...
	public void setFormat(SimpleDateFormat simpleDateFormat)
	{
		m_format = DisplayType.getDateFormat(m_displayType);
		if (m_displayType == DisplayType.Date)
			m_text.setDocument(new MDocDate(m_displayType, simpleDateFormat, m_text, m_title));
	} // setFormat
	 ...
	 * 
	 * @param value
	 *            timestamp/date or String to be parsed
	 */
	...
	public void focusLost(FocusEvent e)
	{
		...
		{
			// teo_sarca [ 1660595 ] Date field: incorrect functionality on
			// paste
			// setValue(startCalendar(this, getTimestamp(), m_format,
			// m_displayType, m_title));
	 ...
	 */
	public static Timestamp startCalendar(Container jc, Timestamp value, SimpleDateFormat format, int displayType,
			String title)
	{
	 ...
	 *
	 * protected void fireActionPerformed() { int modifiers = 0; AWTEvent
	 * currentEvent = EventQueue.getCurrentEvent(); if (currentEvent instanceof
	 * InputEvent) modifiers = ((InputEvent)currentEvent).getModifiers(); else
	 * if (currentEvent instanceof ActionEvent) modifiers =
	 * ((ActionEvent)currentEvent).getModifiers(); ActionEvent ae = new
	 * ActionEvent (this, ActionEvent.ACTION_PERFORMED, "VDate",
	 * EventQueue.getMostRecentEventTime(), modifiers);
	 * 
	 * // Guaranteed to return a non-null array Object[] listeners =
	 * listenerList.getListenerList(); // Process the listeners last to first,
	 * notifying those that are interested in this event for (int i =
	 * listeners.length-2; i>=0; i-=2) { if (listeners[i]==ActionListener.class)
