 ...
 * Lookup Visual Field.
 * <p>
 * When r/o - display a Label When STABLE - display a ComboBox Otherwise show
 * Selection Dialog
 * <p>
 ...
 *         <li>BF [ 1834399 ] VLookup: pressing enter twice has a annoying
 *         behaviour
 ...
 */
public class ZLookup extends JComponent implements VEditor, ActionListener, FocusListener
{
	/**
	 * 
	 */
	...
	final class VLookup_mouseAdapter extends java.awt.event.MouseAdapter
	{
		 ...
		 *            adaptee
		 */
		VLookup_mouseAdapter(ZLookup adaptee)
		{
			m_adaptee = adaptee;
		} // VLookup_mouseAdapter

		private ZLookup m_adaptee;
	 ...
	 * @return VLookup
	 */
	public static ZLookup createBPartner(int WindowNo)
	{
		int AD_Column_ID = 3499; // C_Invoice.C_BPartner_ID
		try
		{
			Lookup lookup = MLookupFactory.get(Env.getCtx(), WindowNo, 0, AD_Column_ID, DisplayType.Search);
			return new ZLookup("C_BPartner_ID", false, false, true, lookup);
		} catch (Exception e)
		{
	 ...
	 * @return VLookup
	 */
	public static ZLookup createProduct(int WindowNo)
	{
		int AD_Column_ID = 3840; // C_InvoiceLine.M_Product_ID
		try
		{
			Lookup lookup = MLookupFactory.get(Env.getCtx(), WindowNo, 0, AD_Column_ID, DisplayType.Search);
			return new ZLookup("M_Product_ID", false, false, true, lookup);
		} catch (Exception e)
		{
			log.log(Level.SEVERE, "", e);
		}
	 ...
	 * @return VLookup
	 */
	public static ZLookup createUser(int WindowNo)
	{
		int AD_Column_ID = 10443; // AD_WF_Activity.AD_User_UD
		try
		{
			Lookup lookup = MLookupFactory.get(Env.getCtx(), WindowNo, 0, AD_Column_ID, DisplayType.Search);
			return new ZLookup("AD_User_ID", false, false, true, lookup);
		} catch (Exception e)
		{
			log.log(Level.SEVERE, "", e);
		}
	 ...
	 *            lookup
	 */
	public ZLookup(String columnName, boolean mandatory, boolean isReadOnly, boolean isUpdateable, Lookup lookup)
	{
		super();
		...
		// *** VComboBox ***
		if (m_lookup != null && m_lookup.getDisplayType() != DisplayType.Search) // No
																					// Search
		{
		...
		if (m_lookup != null)
		{
			if ((m_lookup.getDisplayType() == DisplayType.List && Env.getContextAsInt(Env.getCtx(), "#AD_Role_ID") == 0)
					|| m_lookup.getDisplayType() != DisplayType.List) // only
																		// system
																		// admins
																		// can
																		// change
																		...
																		// others
			{
				if (m_enableInfo && !m_hasButton) // Enable the info window from
													// the pop-up menu if there
													// is no button
				{
		...
		if (columnName.equals("C_BPartner_ID"))
		{
			...
			mBPartnerNew.setVisible(isReadWrite()); // visible only if the field
													// is editable - teo_sarca [
													// 1721710 ]
	...
	/** Logger */
	private static CLogger log = CLogger.getCLogger(ZLookup.class);

	/**
	 * Set Content and Size of Components
	 * 
	 * @param initial
	 *            if true, size and margins will be set
	 */
	private void setUI(boolean initial)
	{
		if (initial)
		{
			...
			setPreferredSize(new Dimension(size)); // causes r/o to be the same
													// length
		...
		// What to show
		this.remove(m_combo); // Need to attach m_combo to a parent for event
								// processing in info panels.
		...
		if (!isReadWrite())
		{
			...
			this.add(m_combo, BorderLayout.SOUTH); // Need to attache m_combo to
													// "this" so it has a parent
			...
			m_comboActive = false;
		} else if (m_lookup != null && m_lookup.getDisplayType() != DisplayType.Search) // show
																						// combo
																						// if
																						// not
																						// Search
	...
	public void setReadWrite(boolean value)
	{
		...
		// If the field is readonly the BPartner new option should be hidden -
		// teo_sarca [ 1721710 ]
	...
	public String getDisplay()
	{
		...
		// log.fine( "VLookup.getDisplay - " + retValue, "ComboActive=" +
		// m_comboActive);
		return retValue;
	} // getDisplay
	...
	public void setField(GridField mField)
	{
		...
		if (mField != null && mField.isAutocomplete() && m_lookup instanceof MLookup
	...
	protected void actionCombo(Object value)
	{
		...
		if (!updated)
		{
			// happens if VLookup is used outside of APanel/GridController (no
			// property listener)
			log.fine(m_columnName + " - Value explicitly set - new=" + updatedValue + ", old=" + m_value);

			// phib: the following check causes the update to fail on jre >
			// 1.6.0_13
	...
	private void actionButton(String queryValue)
	{
		...
		/**
		 * Three return options: - Value Selected & OK pressed => store result
		 * => result has value - Cancel pressed => store null => result == null
		 * && cancelled - Window closed -> ignore => result == null &&
		 * !cancelled
		 */
		...
		//
		// If the record has a value (ID) find the name. The displayed text
		// could be different.
		...
		//
		boolean resetValue = false; // reset value so that is always treated as
									// new entry
		...
		if (infoFactoryClass != null && infoFactoryClass.trim().length() > 0)
		{
			try
			{
				Class<InfoFactory> clazz = (Class<InfoFactory>) this.getClass().getClassLoader()
						.loadClass(infoFactoryClass);
				...
				if (m_tableName == null) // sets table name & key column
				{
					if (!hasSearchableColumns())
					{
						...
						log.severe("Search enabled on field " + m_columnName
								+ ". Associated table has no standard/identifier columns.");
						return;
					}
				}
				// multipleSelection assumed false for custom info windows
				Info ig = factory.create(frame, true, m_lookup.getWindowNo(), m_tableName, m_keyColumnName, record_id,
		...
		} else if (col.equals("M_Product_ID"))
		{
			...
			if (m_mField != null)
			{
				...
				multipleSelection = (MOrderLine.Table_ID == AD_Table_ID) || (MInvoiceLine.Table_ID == AD_Table_ID)
						|| (I_PP_Product_BOMLine.Table_ID == AD_Table_ID) || (MProductPrice.Table_ID == AD_Table_ID);
			}
			// Show Info
			InfoProduct ip = new InfoProduct(frame, true, m_lookup.getWindowNo(), M_Warehouse_ID, M_PriceList_ID,
					record_id, queryValue, multipleSelection, true, whereClause);
			ip.setVisible(true);
		...
		} else if (col.equals("C_BPartner_ID"))
		{
			...
			//
			InfoBPartner ip = new InfoBPartner(frame, true, m_lookup.getWindowNo(), record_id, queryValue, m_isSOTrx,
					m_isSOMatch, multipleSelection, true, whereClause);
			ip.setVisible(true);
			cancelled = ip.isCancelled();
			result = ip.getSelectedKeys();
		} else // General Info
		{
			...
			//
			Info ig = Info.create(frame, true, m_lookup.getWindowNo(), m_tableName, m_keyColumnName, record_id,
					queryValue, multipleSelection, true, whereClause);
			ig.setVisible(true);
			cancelled = ig.isCancelled();
			result = ig.getSelectedKeys();
		}
		...
		if (isReadWrite())
		{
			...
			{
				log.config(
						m_columnName + " - Result = " + result.toString() + " (" + result.getClass().getName() + ")");
	...
	private String getWhereClause()
	{
		...
		// log.finest("ZoomQuery=" + (m_lookup.getZoomQuery()==null ? "" :
		// m_lookup.getZoomQuery().getWhereClause())
	...
	private void actionText()
	{
		...
		m_value = null; // forces re-display if value is unchanged but text
						// updated and still unique
		resetTabInfo();
	...
	private boolean hasSearchableColumns()
	{
		...
		if (m_columnName.equals("M_Product_ID") || m_columnName.equals("M_ProductBOM_ID")
				|| m_columnName.equals("C_BPartner_ID") || m_columnName.equals("C_Order_ID")
				|| m_columnName.equals("C_Invoice_ID") || m_columnName.equals("M_InOut_ID")
				|| m_columnName.equals("C_Payment_ID") || m_columnName.equals("GL_JournalBatch_ID")
		...
		} else
		{
			...
			String query = "SELECT t.TableName, c.ColumnName " + "FROM AD_Column c "
	...
	/**
	 * Generate Access SQL for Search. The SQL returns the ID of the value
	 * entered
	 * 
	 * @param text
	 *            uppercase text for LIKE comparison
	 * @return sql or "" Example SELECT C_Payment_ID FROM C_Payment WHERE
	 *         UPPER(DocumentNo) LIKE x OR ...
	 */
	private String getDirectAccessSQL(String text)
	{
		...
		if (m_columnName.equals("M_Product_ID"))
		{
			...
			{
				sql.append("UPPER(Value) LIKE ").append(DB.TO_STRING(text)).append(" OR UPPER(Name) LIKE ")
						.append(DB.TO_STRING(text)).append(" OR UPPER(SKU) LIKE ").append(DB.TO_STRING(text))
		...
		} else if (m_columnName.equals("C_BPartner_ID"))
		{
			...
			if (!text.matches(".*\\D+.*")) // If text has no non-digit
											// characters ...
				// search against the Value field
				sql.append("UPPER(Value) LIKE ").append(DB.TO_STRING(text));
			else
				// A few non-digit characters might be in the name. E.g.
				// 451Group, 1st Choice, ...
		...
		if (m_lookup != null && m_lookup instanceof MLookup)
		{
			...
			{
				String query = "SELECT kc.ColumnName, dc.ColumnName, t.TableName " + "FROM AD_Ref_Table rt"
						+ " INNER JOIN AD_Column kc ON (rt.AD_Key=kc.AD_Column_ID)"
						+ " INNER JOIN AD_Column dc ON (rt.AD_Display=dc.AD_Column_ID)"
						+ " INNER JOIN AD_Table t ON (rt.AD_Table_ID=t.AD_Table_ID) " + "WHERE rt.AD_Reference_ID=?";
				...
				if (displayColumnName != null)
				{
					...
					sql.append("SELECT ").append(m_keyColumnName).append(" FROM ").append(m_tableName)
							.append(" WHERE UPPER(").append(displayColumnName).append(") LIKE ")
							.append(DB.TO_STRING(text)).append(" AND IsActive='Y'");
					String wc = getWhereClause();
					if (wc != null && wc.length() > 0)
						sql.append(" AND ").append(wc);
					// ***
					log.finest(m_columnName + " (Table) " + sql.toString());
					return MRole.getDefault().addAccessSQL(sql.toString(), m_tableName, MRole.SQL_NOTQUALIFIED,
							MRole.SQL_RO);
				}
			} // Table Reference
		} // MLookup
		...
		//
		StringBuffer retValue = new StringBuffer("SELECT ").append(m_columnName).append(" FROM ").append(m_tableName)
				.append(" WHERE (").append(sql).append(")").append(" AND IsActive='Y'");
		String wc = getWhereClause();
		if (wc != null && wc.length() > 0)
	...
	private void actionZoom(Object selectedItem)
	{
		...
		{
			zoomQuery = new MQuery(); // ColumnName might be changed in
										// MTab.validateQuery
			...
			if (m_lookup != null && m_lookup instanceof MLookup)
			{
				...
				if (DisplayType.List == m_lookup.getDisplayType())
				{
					...
					value = DB.getSQLValue(null,
							"SELECT AD_Ref_List_ID FROM AD_Ref_List WHERE AD_Reference_ID=? AND Value=?",
							AD_Reference_ID, value);
				} else
				{
					if (AD_Reference_ID != 0)
					{
						String query = "SELECT kc.ColumnName, kt.TableName" + " FROM AD_Ref_Table rt"
								+ " INNER JOIN AD_Column kc ON (rt.AD_Key=kc.AD_Column_ID)"
	...
	public void focusGained(FocusEvent e)
	{
		if (m_combo == null || m_combo.getEditor() == null)
			return;
		if ((e.getSource() != m_combo && e.getSource() != m_combo.getEditor().getEditorComponent()) || e.isTemporary()
				|| m_haveFocus || m_lookup == null)
			return;
		...
		m_lookup.fillComboBox(isMandatory(), true, true, false); // only
																	// validated
																	// & active
		if (popupVisible)
		{
		...
		log.config(m_columnName + " - Update   Count=" + m_combo.getItemCount() + ", Selected="
				+ m_lookup.getSelectedItem());
		m_lookup.setSelectedItem(obj);
		log.config(m_columnName + " - Selected Count=" + m_combo.getItemCount() + ", Selected="
				+ m_lookup.getSelectedItem());
		//
	...
	{
		if (e.isTemporary() || m_lookup == null || !m_button.isEnabled()) // set
																			// by
																			// actionButton
			return;
		// Text Lost focus
		...
		Object obj = m_combo.getSelectedItem();
		/*
		 * // set original model if (!m_lookup.isValidated())
	...
	/**
	 * Use by vcelleditor to indicate editing is off and don't invoke
	 * databinding
	 * 
	 * @param stopediting
	 */
	...
	{
		if (m_lookup instanceof MLookup && m_lookup.getDisplayType() == DisplayType.Search)
		{
			new ZLookupAutocompleter(this.m_text, this, (MLookup) m_lookup);
		}
	}

	/**
	 * @param override
	 *            - true to override the environment, false to use environment
	 * @param trx
	 *            the m_isSOTrx to set
	 */
	public void setIsSOTrx(boolean override, boolean trx)
	{
		...
		if (m_isSOTrxEnvOverride)
			m_isSOTrx = trx;
		else if (Env.getContext(Env.getCtx(), m_lookup.getWindowNo(), "IsSOTrx").equals("N"))
			m_isSOTrx = false;
		else
			m_isSOTrx = true;
	}
