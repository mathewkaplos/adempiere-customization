...
import org.zenith.model.payment.ConsolidateOrder;
 ...
 *
 * @author Carlos Ruiz - globalqss [ 2141475 ] Payment <> allocations must not
 *         be completed - implement lots of validations on prepareIt
...
public final class MPayment extends X_C_Payment implements DocAction, ProcessCall
{
	...
	public static MPayment[] getOfBPartner(Properties ctx, int C_BPartner_ID, String trxName)
	{
		...
		List<MPayment> list = new Query(ctx, I_C_Payment.Table_Name, whereClause, trxName).setParameters(C_BPartner_ID)
				.list();
	 ...
	 */
	public boolean setCreditCard(String TrxType, String creditCardType, String creditCardNumber, String creditCardVV,
	...
	{
		return setCreditCard(TrxType, creditCardType, creditCardNumber, creditCardVV,
				MPaymentValidate.getCreditCardExpMM(creditCardExp), MPaymentValidate.getCreditCardExpYY(creditCardExp));
	} // setCreditCard
	 ...
	 * @return true if valid
	 */
	public boolean setBankACH(int C_BankAccount_ID, boolean isReceipt, String tenderType, String routingNo,
			String accountNo)
	{
	 ...
	 * @return true if valid
	 */
	public boolean setBankCheck(int C_BankAccount_ID, boolean isReceipt, String routingNo, String accountNo,
			String checkNo)
	{
		setTenderType(TENDERTYPE_Check);
		setIsReceipt(isReceipt);
	...
	public void setBankAccountDetails(int C_BankAccount_ID)
	{
		...
		setC_BankAccount_ID(C_BankAccount_ID);
		//
		String sql = "SELECT b.RoutingNo, ba.AccountNo " + "FROM C_BankAccount ba"
				+ " INNER JOIN C_Bank b ON (ba.C_Bank_ID=b.C_Bank_ID) " + "WHERE C_BankAccount_ID=?";
	...
	protected boolean beforeSave(boolean newRecord)
	{
		...
		if (newRecord || is_ValueChanged("C_Charge_ID") || is_ValueChanged("C_Invoice_ID")
				|| is_ValueChanged("C_Order_ID") || is_ValueChanged("C_Project_ID"))
			setIsPrepayment(getC_Charge_ID() == 0 && getC_BPartner_ID() != 0
		...
		// [ adempiere-Bugs-1885417 ] Validate BP on Payment Prepare or
		// BeforeSave
	...
	public BigDecimal getAllocatedAmt()
	{
		...
		String sql = "SELECT SUM(currencyConvert(al.Amount,"
				...
				+ " INNER JOIN C_Payment p ON (al.C_Payment_ID=p.C_Payment_ID) " + "WHERE al.C_Payment_ID=?"
	...
	public boolean testAllocation()
	{
		...
		/*
		 * @Trifon - CashPayments if (isCashTrx()) { if (!isAllocated()) {
		 * setIsAllocated(true); return true; } return false; }
		 */
		//
	...
	public ValueNamePair[] getCreditCards(BigDecimal amt)
	{
		...
		{
			if (m_mPaymentProcessors == null || m_mPaymentProcessors.length == 0)
				m_mPaymentProcessors = MPaymentProcessor.find(getCtx(), null, null, getAD_Client_ID(),
						getC_Currency_ID(), amt, get_TrxName());
			//
	...
	private void setDocumentNo()
	{
		...
		documentNo = "";
		// globalqss - read configuration to assign credit card or check number
		// number for Payments
		// Credit Card
		if (TENDERTYPE_CreditCard.equals(getTenderType()))
		{
			...
			{
				documentNo = getCreditCardType() + " " + Obscure.obscure(getCreditCardNumber()) + " "
						+ getCreditCardExpMM() + "/" + getCreditCardExpYY();
			}
		}
		// Own Check No
		else if (TENDERTYPE_Check.equals(getTenderType()) && !isReceipt() && getCheckNo() != null
				&& getCheckNo().length() > 0)
		{
			if (MSysConfig.getBooleanValue("PAYMENT_OVERWRITE_DOCUMENTNO_WITH_CHECK_ON_PAYMENT", true,
					getAD_Client_ID()))
			{
		...
		{
			if (MSysConfig.getBooleanValue("PAYMENT_OVERWRITE_DOCUMENTNO_WITH_CHECK_ON_RECEIPT", true,
					getAD_Client_ID()))
			{
	...
	public void setBP_BankAccount(MBPBankAccount ba)
	{
		...
		setAccountAddress(ba.getA_Name(), ba.getA_Street(), ba.getA_City(), ba.getA_State(), ba.getA_Zip(),
				ba.getA_Country());
	...
	private boolean verifyDocType(MPaymentAllocate[] pAllocs)
	{
		...
		if (documentSO != null && documentSO.booleanValue() != paymentSO.booleanValue())
			return false;
		// OK
		return true;
	} // verifyDocType
	...
	public String prepareIt()
	{
		...
		if (getC_Order_ID() == 0 && getC_Invoice_ID() == 0 && isPrepayment() == false && isInvoice() == false)
		{ // see
			// WebOrder.process
			int b = ConsolidateOrder.copyOrder(getCtx(), getC_BPartner_ID(), get_TrxName());
			System.out.println("bbbbbbbbbbbbbbbb=" + b);
			// MOrder order = new MOrder(getCtx(), b, get_TrxName());
			...
			if (DOCSTATUS_WaitingPayment.equals(order.getDocStatus()))
			{
				...
				if (getC_Invoice_ID() == 0)
				{
					System.out.println("pretty 380");
					m_processMsg = "@NotFound@ @C_Invoice_ID@---380";
					return DocAction.STATUS_Invalid;
				}
			} // WaitingPayment
		}
		// Waiting Payment - Need to create Invoice & Shipment
		else if (getC_Order_ID() != 0 && getC_Invoice_ID() == 0)
		{ // see WebOrder.process
			MOrder order = new MOrder(getCtx(), getC_Order_ID(), get_TrxName());
			if (DOCSTATUS_WaitingPayment.equals(order.getDocStatus()))
			{
			...
			// Pre payment
			// mathew
		} else if (getC_Order_ID() == 0 && getC_Invoice_ID() == 0 && isPrepayment() == false && isInvoice() == true)
		{
			createALlocations(this);
		}
	...
	public String completeIt()
	{
				...
				&& MPaymentAllocate.get(this).length == 0)
		{
			...
			BigDecimal payAmt = MConversionRate.convertBase(getCtx(), getPayAmt(), getC_Currency_ID(), getDateAcct(),
		...
		if (isCashTrx() && !MSysConfig.getBooleanValue("CASH_AS_PAYMENT", true, getAD_Client_ID()))
		{
			...
			// MCash cash = MCash.get (getCtx(), getAD_Org_ID(), getDateAcct(),
			// getC_Currency_ID(), get_TrxName());SHW
	...
	private MPayment createCounterDoc()
	{
		...
		String sql = "SELECT C_BankAccount_ID FROM C_BankAccount "
				+ "WHERE C_Currency_ID=? AND AD_Org_ID IN (0,?) AND IsActive='Y' " + "ORDER BY IsDefault DESC";
	...
	public boolean allocateIt()
	{
		...
		MAllocationHdr alloc = new MAllocationHdr(getCtx(), false, getDateTrx(), getC_Currency_ID(),
		...
		for (int i = 0; i < pAllocs.length; i++)
		{
			...
			if (isReceipt())
				aLine = new MAllocationLine(alloc, pa.getAmount(), pa.getDiscountAmt(), pa.getWriteOffAmt(),
						pa.getOverUnderAmt());
			else
				aLine = new MAllocationLine(alloc, pa.getAmount().negate(), pa.getDiscountAmt().negate(),
						pa.getWriteOffAmt().negate(), pa.getOverUnderAmt().negate());
	...
	private boolean allocateInvoice()
	{
		...
		if (getOverUnderAmt().signum() < 0 && getPayAmt().signum() > 0)
			allocationAmt = allocationAmt.add(getOverUnderAmt()); // overpayment
																	// (negative)

		MAllocationHdr alloc = new MAllocationHdr(getCtx(), false, getDateTrx(), getC_Currency_ID(),
				Msg.translate(getCtx(), "C_Payment_ID") + ": " + getDocumentNo() + " [1]", get_TrxName());
		alloc.setAD_Org_ID(getAD_Org_ID());
		alloc.setDateAcct(getDateAcct()); // in case date acct is different from
											// datetrx in payment
		alloc.saveEx();
		MAllocationLine aLine = null;
		if (isReceipt())
			...
			aLine = new MAllocationLine(alloc, allocationAmt.negate(), getDiscountAmt().negate(),
					getWriteOffAmt().negate(), getOverUnderAmt().negate());
		...
		int C_Project_ID = DB.getSQLValue(get_TrxName(), "SELECT MAX(C_Project_ID) FROM C_Invoice WHERE C_Invoice_ID=?",
				getC_Invoice_ID());
	...
	private boolean allocatePaySelection()
	{
		...
		String sql = "SELECT psc.C_BPartner_ID, psl.C_Invoice_ID, psl.IsSOTrx, " // 1..3
				+ " psl.PayAmt, psl.DiscountAmt, psl.DifferenceAmt, psl.OpenAmt " + "FROM C_PaySelectionLine psl"
		...
		try
		{
			...
			while (rs.next())
			{
				...
				if (isSOTrx)
					...
					aLine = new MAllocationLine(alloc, PayAmt.negate(), DiscountAmt.negate(), WriteOffAmt.negate(),
							OverUnderAmt.negate());
	...
	private void deAllocate()
	{
		...
		{
			// Invoice
			String sql = "UPDATE C_Invoice " + "SET C_Payment_ID = NULL, IsPaid='N' " + "WHERE C_Invoice_ID="
					+ getC_Invoice_ID() + " AND C_Payment_ID=" + getC_Payment_ID();
			int no = DB.executeUpdate(sql, get_TrxName());
			if (no != 0)
				log.fine("Unlink Invoice #" + no);
			// Order
			sql = "UPDATE C_Order o " + "SET C_Payment_ID = NULL " + "WHERE EXISTS (SELECT * FROM C_Invoice i "
	...
	public String toString()
	{
		...
		sb.append(get_ID()).append("-").append(getDocumentNo()).append(",Receipt=").append(isReceipt())
				.append(",PayAmt=").append(getPayAmt()).append(",Discount=").append(getDiscountAmt())
				.append(",WriteOff=").append(getWriteOffAmt()).append(",OverUnder=").append(getOverUnderAmt());
		return sb.toString();
	} // toString
	...
	{
		Object oo = get_Value("is_invoice");
		if (oo != null)
		{
	...
	/*
	 * 
	 * Create allocations from the unpaid invoices mathew
	 */
	private void createALlocations(MPayment parent)
	{
		...
		query.setParameters(parent.getC_BPartner_ID(), "CO", false).setOrderBy("C_Invoice_ID")
		...
		BigDecimal amt = parent.getPayAmt();
		for (int i = 0; i < invoices.length; i++)
		{
			...
			if (amt.compareTo(bd) >= 0)
			{
				...
				amt = amt.subtract(bd);
				// inv.setIsPaid(true);
				inv.setC_Payment_ID(parent.get_ID());
				inv.save();
				System.out.println("------------------------------");
			} else
			{
	...
	public void setTenderAmt(BigDecimal tender_amt)
	{
		set_Value("tender_amt", tender_amt);
	}

	public BigDecimal getTenderAmt()
	{
		BigDecimal bd = (BigDecimal) get_Value("tender_amt");
		if (bd == null)
		{
			return Env.ZERO;
		}
