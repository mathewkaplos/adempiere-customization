...
public class MInvoice extends X_C_Invoice implements DocAction
{
	...
	/** Cache */
	private static CCache<Integer, MInvoice> s_cache = new CCache<Integer, MInvoice>("C_Invoice", 20, 2); // 2
																											// minutes
	...
	public void setBPartner(MBPartner bp)
	{
		...
		if (getC_BPartner_Location_ID() == 0)
			log.log(Level.SEVERE, new BPartnerNoAddressException(bp).getLocalizedMessage()); // TODO:
																								// throw
																								// exception?
	...
	public void setProcessed(boolean processed)
	{
		...
		if (get_ID() == 0)
			return;
		String set = "SET Processed='" + (processed ? "Y" : "N") + "' WHERE C_Invoice_ID=" + getC_Invoice_ID();
	...
	{
		StringBuffer sb = new StringBuffer("MInvoice[").append(get_ID()).append("-").append(getDocumentNo())
	...
	protected boolean afterSave(boolean newRecord, boolean success)
	{
		...
		{
			String sql = "UPDATE C_InvoiceLine ol" + " SET AD_Org_ID =" + "(SELECT AD_Org_ID"
					+ " FROM C_Invoice o WHERE ol.C_Invoice_ID=o.C_Invoice_ID) " + "WHERE C_Invoice_ID="
					+ getC_Invoice_ID();
			...
			for (int i = 0; i < lines.length; i++)
			{
				...
				if (bom != null)
				{
					...
					for (int j = 0; j < bomlines.length; j++)
					{
						...
						newLine.setQty(line.getQtyInvoiced().multiply(bomline.getQtyBOM())); // Invoiced/Entered
	...
	public boolean calculateTaxTotal()
	{
		...
		for (int i = 0; i < lines.length; i++)
		{
			...
			{
				MInvoiceTax iTax = MInvoiceTax.get(line, getPrecision(), false, get_TrxName()); // current
																								// Tax
				if (iTax != null)
				{
	...
	public String completeIt()
	{
		...
		BigDecimal invAmt = MConversionRate.convertBase(getCtx(), getGrandTotal(true), // CM
																						// adjusted
		...
		else
		{
			...
			log.fine("GrandTotal=" + getGrandTotal(true) + "(" + invAmt + ") Balance=" + bp.getTotalOpenBalance(false)
					+ " -> " + newBalance);
		}
		...
		if (isSOTrx() && getC_Project_ID() != 0)
		{
			...
			if (amt == null)
			{
				m_processMsg = "Could not convert C_Currency_ID=" + getC_Currency_ID() + " to Project C_Currency_ID="
						+ C_CurrencyTo_ID;
				return DocAction.STATUS_Invalid;
			}
			...
			log.fine("GrandTotal=" + getGrandTotal(true) + "(" + amt + ") Project " + project.getName() + " - Invoiced="
	...
	public boolean reverseCorrectIt()
	{
		...
		// Deep Copy
		MInvoice reversal = copyFrom(this, getDateInvoiced(), getDateAcct(), getC_DocType_ID(), isSOTrx(), false,
				get_TrxName(), true);
		if (reversal == null)
		{
		...
		// Create Allocation
		MAllocationHdr alloc = new MAllocationHdr(getCtx(), false, getDateAcct(), getC_Currency_ID(),
