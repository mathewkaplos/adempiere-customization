 ...
 */
public class MInOutLine extends X_M_InOutLine implements IDocumentLine
{
	/**
	 *
	 */
	 ...
	 *            line
	 * @param where
	 *            optional addition where clause
	 * @param trxName
	 *            transaction
	...
	public static MInOutLine[] getOfOrderLine(Properties ctx, int C_OrderLine_ID, String where, String trxName)
	{
		...
		List<MInOutLine> list = new Query(ctx, Table_Name, whereClause, trxName).setParameters(C_OrderLine_ID).list();
	...
	public static MInOutLine[] getOfRMALine(Properties ctx, int M_RMALine_ID, String where, String trxName)
	{
		...
		List<MInOutLine> list = new Query(ctx, Table_Name, whereClause, trxName).setParameters(M_RMALine_ID).list();
		return list.toArray(new MInOutLine[list.size()]);
	} // getOfRMALine
	...
	public void setM_Locator_ID(BigDecimal Qty)
	{
		...
		int M_Locator_ID = MStorage.getM_Locator_ID(getM_Warehouse_ID(), getM_Product_ID(),
	...
	protected boolean beforeSave(boolean newRecord)
	{
		...
		if (getM_Locator_ID() > 0)
		{
			...
			{
				throw new WarehouseLocatorConflictException(MWarehouse.get(getCtx(), getM_Warehouse_ID()), locator,
						getLine());
			}
		}
		...
		/**
		 * Qty on instance ASI if (getM_AttributeSetInstance_ID() != 0) {
		 * MProduct product = getProduct(); int M_AttributeSet_ID =
		 * product.getM_AttributeSet_ID(); boolean isInstance =
		 * M_AttributeSet_ID != 0; if (isInstance) { MAttributeSet mas =
		 * MAttributeSet.get(getCtx(), M_AttributeSet_ID); isInstance =
		 * mas.isInstanceAttribute(); } // Max if (isInstance) { MStorage
		 * storage = MStorage.get(getCtx(), getM_Locator_ID(),
		 * getM_Product_ID(), getM_AttributeSetInstance_ID(), get_TrxName()); if
		 * (storage != null) { BigDecimal qty = storage.getQtyOnHand(); if
		 * (getMovementQty().compareTo(qty) > 0) { log.warning("Qty - Stock=" +
		 * qty + ", Movement=" + getMovementQty());
		 * log.saveError("QtyInsufficient", "=" + qty); return false; } } } } /
		 **/
	...
	private void updateTotalAmount()
	{
		...
		BigDecimal grandTotal = getTotals(getM_InOut_ID());
		MInOut inOut = new MInOut(getCtx(), getM_InOut_ID(), get_TrxName());
		inOut.setTotalAmt(grandTotal);
		inOut.save();

	}

	private BigDecimal getTotals(int m_InOut_ID)
	{
		String sql = "SELECT SUM(LineNetAmt) FROM adempiere.M_Inoutline WHERE M_InOut_ID= " + m_InOut_ID;
		BigDecimal value = DB.getSQLValueBD(get_TrxName(), sql);
		return value;
	}
	...
	{
		StringBuffer sb = new StringBuffer("MInOutLine[").append(get_ID()).append(",M_Product_ID=")
				.append(getM_Product_ID()).append(",QtyEntered=").append(getQtyEntered()).append(",MovementQty=")
				.append(getMovementQty()).append(",M_AttributeSetInstance_ID=").append(getM_AttributeSetInstance_ID())
				.append("]");
		return sb.toString();
	} // toString
	...
	public BigDecimal getBase(String CostDistribution)
	{
		if (MLandedCost.LANDEDCOSTDISTRIBUTION_Costs.equals(CostDistribution))
		{
			...
			if (m_il == null)
			{
				...
				MMatchPO matchPO = new Query(this.getCtx(), I_M_MatchPO.Table_Name, whereClause.toString(),
						this.get_TrxName()).setParameters(this.getC_OrderLine_ID(), this.getM_Product_ID()).first();
				if (matchPO != null)
			...
			return this.getMovementQty().multiply(m_il.getPriceActual()); // Actual
																			// delivery
		...
		if (getC_OrderLine_ID() > 0)
		{
			price = DB.getSQLValueBDEx(get_TrxName(),
					"SELECT currencyBase(ol.PriceActual,o.C_Currency_ID,o.DateAcct,o.AD_Client_ID,o.AD_Org_ID) AS price "
							+ " FROM C_OrderLine ol INNER JOIN C_Order o ON (o.C_Order_ID=ol.C_Order_ID) " + " WHERE "
							+ MOrderLine.COLUMNNAME_C_OrderLine_ID + "=?",
					getC_OrderLine_ID());
			...
			if (price == null || price.signum() == 0)
				price = DB.getSQLValueBDEx(get_TrxName(),
						" SELECT currencyBase(ol.PriceActual,o.C_Currency_ID,o.DateAcct,o.AD_Client_ID,o.AD_Org_ID) AS price"
								...
								+ " INNER JOIN C_Order o ON (o.C_Order_ID=ol.C_Order_ID) " + " WHERE  mpo."
								+ MMatchPO.COLUMNNAME_M_InOutLine_ID + "=?",
						getM_InOutLine_ID());
			...
			if (price == null || price.signum() == 0)
				price = DB.getSQLValueBDEx(get_TrxName(),
						" SELECT currencyBase(il.PriceActual,i.C_Currency_ID,i.DateAcct,i.AD_Client_ID,i.AD_Org_ID) AS price "
								...
								+ " INNER JOIN C_Invoice i ON (i.C_Invoice_ID=il.C_Invoice_ID) " + " WHERE  mi."
								+ MMatchInv.COLUMNNAME_M_InOutLine_ID + "=?",
						getM_InOutLine_ID());
		}
		if (getM_RMALine_ID() > 0)
		{
			price = DB.getSQLValueBDEx(get_TrxName(), "SELECT " + MRMALine.COLUMNNAME_Amt + " FROM "
					+ MRMALine.Table_Name + " WHERE " + MRMALine.COLUMNNAME_M_RMALine_ID + "=?", getM_RMALine_ID());
		}
		if (price == null)
		{
