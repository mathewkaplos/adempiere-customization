...
import org.zenith.model.strategy.fromcashier.CashierContext;
 ...
 *      <li>BF ADEMPIERE-79 The Sales Order / Purchase Order ignore the excluded
 *      tables for an Attribute Set defined as always mandatory
...
public class MOrder extends X_C_Order implements DocAction
{
	 ...
	 * @return Order
	 */
	public static MOrder copyFrom(MOrder from, Timestamp dateDoc, int C_DocTypeTarget_ID, boolean isSOTrx,
			boolean counter, boolean copyASI, String trxName)
	{
	...
	public void setC_DocTypeTarget_ID(String DocSubTypeSO_x)
	{
		String sql = "SELECT C_DocType_ID FROM C_DocType " + "WHERE AD_Client_ID=? AND AD_Org_ID IN (0,"
				+ getAD_Org_ID() + ") AND DocSubTypeSO=? " + " AND IsActive='Y' "
	...
	public void setC_DocTypeTarget_ID()
	{
		...
		// PO
		String sql = "SELECT C_DocType_ID FROM C_DocType " + "WHERE AD_Client_ID=? AND AD_Org_ID IN (0,"
				+ getAD_Org_ID() + ") AND DocBaseType='POO' " + "ORDER BY AD_Org_ID DESC, IsDefault DESC";
	...
	{
		StringBuffer sb = new StringBuffer("MOrder[").append(get_ID()).append("-").append(getDocumentNo())
				.append(",IsSOTrx=").append(isSOTrx()).append(",C_DocType_ID=").append(getC_DocType_ID())
				.append(", GrandTotal=").append(getGrandTotal()).append("]");
		return sb.toString();
	} // toString
	...
	public MOrderLine[] getLines(String whereClause, String orderClause)
	{
		...
		//
		List<MOrderLine> list = new Query(getCtx(), I_C_OrderLine.Table_Name, whereClauseFinal.toString(),
	...
	public MInvoice[] getInvoices()
	{
		final String whereClause = "EXISTS (SELECT 1 FROM C_InvoiceLine il, C_OrderLine ol"
				+ " WHERE il.C_Invoice_ID=C_Invoice.C_Invoice_ID" + " AND il.C_OrderLine_ID=ol.C_OrderLine_ID"
				+ " AND ol.C_Order_ID=?)";
		List<MInvoice> list = new Query(getCtx(), I_C_Invoice.Table_Name, whereClause, get_TrxName())
				.setParameters(get_ID()).setOrderBy("C_Invoice_ID DESC").list();
	...
	public MInOut[] getShipments()
	{
		final String whereClause = "EXISTS (SELECT 1 FROM M_InOutLine iol, C_OrderLine ol"
				+ " WHERE iol.M_InOut_ID=M_InOut.M_InOut_ID" + " AND iol.C_OrderLine_ID=ol.C_OrderLine_ID"
				+ " AND ol.C_Order_ID=?)";
		List<MInvoice> list = new Query(getCtx(), I_M_InOut.Table_Name, whereClause, get_TrxName())
				.setParameters(get_ID()).setOrderBy("M_InOut_ID DESC").list();
	...
	public void setProcessed(boolean processed)
	{
		...
		if (get_ID() == 0)
			return;
		String set = "SET Processed='" + (processed ? "Y" : "N") + "' WHERE C_Order_ID=" + getC_Order_ID();
	...
	protected boolean afterSave(boolean newRecord, boolean success)
	{
		...
		// TODO: The changes here with UPDATE are not being saved on change log
		// - audit problem

		// Propagate Description changes
		if (is_ValueChanged("Description") || is_ValueChanged("POReference"))
		{
			String sql = "UPDATE C_Invoice i" + " SET (Description,POReference)=" + "(SELECT Description,POReference "
		...
		// invoices
		if (is_ValueChanged("PaymentRule") || is_ValueChanged("C_PaymentTerm_ID") || is_ValueChanged("C_Payment_ID")
				|| is_ValueChanged("C_CashLine_ID"))
		{
			String sql = "UPDATE C_Invoice i " + "SET (PaymentRule,C_PaymentTerm_ID,C_Payment_ID,C_CashLine_ID)="
		...
		// Propagate Changes of Date Account to existing (not
		// completed/reversed/closed) invoices
		if (is_ValueChanged("DateAcct"))
		{
			String sql = "UPDATE C_Invoice i " + "SET (DateAcct)=" + "(SELECT DateAcct "
					+ "FROM C_Order o WHERE i.C_Order_ID=o.C_Order_ID)"
		...
		// Sync Lines
		if (is_ValueChanged("AD_Org_ID") || is_ValueChanged(MOrder.COLUMNNAME_C_BPartner_ID)
				|| is_ValueChanged(MOrder.COLUMNNAME_C_BPartner_Location_ID)
				|| is_ValueChanged(MOrder.COLUMNNAME_DateOrdered) || is_ValueChanged(MOrder.COLUMNNAME_DatePromised)
				|| is_ValueChanged(MOrder.COLUMNNAME_M_Warehouse_ID) || is_ValueChanged(MOrder.COLUMNNAME_M_Shipper_ID)
	...
	public String prepareIt()
	{
		...
		if (getC_DocType_ID() != getC_DocTypeTarget_ID())
		{
			...
			if (getC_DocType_ID() != 0)
			{
				...
				if (MDocType.DOCSUBTYPESO_StandardOrder.equals(dtOld.getDocSubTypeSO()) // From
																						// SO
						&& !MDocType.DOCSUBTYPESO_StandardOrder.equals(dt.getDocSubTypeSO())) // To
																								// !SO
				{
					for (int i = 0; i < lines.length; i++)
					{
		...
		if (!MDocType.DOCBASETYPE_PurchaseOrder.equals(dt.getDocBaseType()))
			isASIMandatory();
		...
		if (isSOTrx())
		{
			if (MDocType.DOCSUBTYPESO_POSOrder.equals(dt.getDocSubTypeSO()) && PAYMENTRULE_Cash.equals(getPaymentRule())
					&& !MSysConfig.getBooleanValue("CHECK_CREDIT_ON_CASH_POS_ORDER", true, getAD_Client_ID(),
							getAD_Org_ID()))
			{
				// ignore -- don't validate for Cash POS Orders depending on
				// sysconfig parameter
			} else if (MDocType.DOCSUBTYPESO_PrepayOrder.equals(dt.getDocSubTypeSO()) && !MSysConfig
					.getBooleanValue("CHECK_CREDIT_ON_PREPAY_ORDER", true, getAD_Client_ID(), getAD_Org_ID()))
			{
				// ignore -- don't validate Prepay Orders depending on sysconfig
				// parameter
			} else
			{
				MBPartner bp = new MBPartner(getCtx(), getBill_BPartner_ID(), get_TrxName()); // bill
																								// bp
																								// is
																								// guaranteed
																								// on
				...
				BigDecimal grandTotal = MConversionRate.convertBase(getCtx(), getGrandTotal(), getC_Currency_ID(),
						getDateOrdered(), getC_ConversionType_ID(), getAD_Client_ID(), getAD_Org_ID());
				if (MBPartner.SOCREDITSTATUS_CreditHold.equals(bp.getSOCreditStatus(grandTotal)))
				{
					m_processMsg = "@BPartnerOverOCreditHold@ - @TotalOpenBalance@=" + bp.getTotalOpenBalance()
		 ...
		 * ol.getM_Product_ID() == 0 || dt.isProposal(),
	...
	private boolean explodeBOM()
	{
		...
		while (count != 0)
		{
			...
			for (int i = 0; i < lines.length; i++)
			{
				 ...
				 * boms[j]; MPPProductBOMLine bom = boms[j]; MOrderLine newLine
				 * = new MOrderLine (this); newLine.setLine (++lineNo);
				 * //newLine.setM_Product_ID (bom.getProduct () //
				 * .getM_Product_ID ()); newLine.setM_Product_ID
				 * (bom.getM_Product_ID ()); //newLine.setC_UOM_ID
				 * (bom.getProduct ().getC_UOM_ID ()); newLine.setC_UOM_ID
				 * (bom.getC_UOM_ID ()); //newLine.setQty (line.getQtyOrdered
				 * ().multiply ( // bom.getBOMQty ())); newLine.setQty
				 * (line.getQtyOrdered ().multiply ( bom.getQtyBOM())); if
				 * (bom.getDescription () != null) newLine.setDescription
				 * (bom.getDescription ()); // newLine.setPrice (); newLine.save
				 * (get_TrxName()); }
				 */
	...
	private boolean reserveStock(MDocType dt, MOrderLine[] lines)
	{
		...
		for (int i = 0; i < lines.length; i++)
		{
			...
			log.fine("Line=" + line.getLine() + " - Target=" + target + ",Difference=" + difference + " - Ordered="
					+ line.getQtyOrdered() + ",Reserved=" + line.getQtyReserved() + ",Delivered="
				...
				if (product.isStocked())
				{
					...
					if (M_Locator_ID == 0)
					{
						...
						if (M_Locator_ID != 0)
						{
							...
							// product has default locator defined but is not
							// from the order warehouse
					...
					// Update Storage
					if (!MStorage.add(getCtx(), line.getM_Warehouse_ID(), M_Locator_ID, line.getM_Product_ID(),
							line.getM_AttributeSetInstance_ID(), line.getM_AttributeSetInstance_ID(), Env.ZERO,
	...
	public boolean calculateTaxTotal()
	{
		...
		for (int i = 0; i < lines.length; i++)
		{
			...
			{
				MOrderTax oTax = MOrderTax.get(line, getPrecision(), false, get_TrxName()); // current
																							// Tax
	...
	public String completeIt()
	{
		...
		System.out.println("DocSubTypeSO 1 " + DocSubTypeSO);
		...
		// Offers
		if (MDocType.DOCSUBTYPESO_Proposal.equals(DocSubTypeSO) || MDocType.DOCSUBTYPESO_Quotation.equals(DocSubTypeSO))
		{
			// Binding
		...
		if (MDocType.DOCSUBTYPESO_OnCreditOrder.equals(DocSubTypeSO) // (W)illCall(I)nvoice
				...
				|| MDocType.DOCSUBTYPESO_StandardOrder.equals(DocSubTypeSO))
		{
	...
	private MInOut createShipment(MDocType dt, Timestamp movementDate)
	{
		...
		// Manually Process Shipment
		String status = "";
		if (dt.getDocSubTypeSO().equalsIgnoreCase(MDocType.DOCSUBTYPESO_POSOrder)
				|| dt.getDocSubTypeSO().equalsIgnoreCase(MDocType.DOCSUBTYPESO_WarehouseOrder)
				|| dt.getDocSubTypeSO().equalsIgnoreCase(MDocType.DOCSUBTYPESO_OnCreditOrder))
		{
		...
		// {
		// MOrderLine oLine = oLines[i];
		// CashierContext cc = new CashierContext(oLine);
		// }
	...
	private MInvoice createInvoice(MDocType dt, MInOut shipment, Timestamp invoiceDate)
	{
		...
		if (this.getTreatmentDocID() > 0)
		{
			invoice.setTreatmentDocID(this.getTreatmentDocID());
			invoice.save();
		}
	...
	private String deleteMatchPOCostDetail(MOrderLine line)
	{
		...
		for (int asn = 0; asn < acctschemas.length; asn++)
		{
			...
			if (mPO.length == 0)
			{
				MCostDetail cd = MCostDetail.get(getCtx(), "C_OrderLine_ID=?", line.getC_OrderLine_ID(),
						line.getM_AttributeSetInstance_ID(), as.getC_AcctSchema_ID(), get_TrxName());
				if (cd != null)
				{
	...
	/**
	 * Checks if the order is fully delivered and if it is the IsDelivered flag
	 * will be updated.
	 * 
	 * @param C_Order_ID
	 */
		...
		for (MOrderLine ol : getLines())
		{
			...
			if (product.getM_AttributeSet_ID() > 0)
			{
				if (product.isASIMandatory(isSOTrx(), getAD_Org_ID()))
				{
					...
					if (!mas.excludeEntry(
							MColumn.getColumn_ID(MOrderLine.Table_Name, MOrderLine.COLUMNNAME_C_OrderLine_ID),
							isSOTrx()) && ol.getM_AttributeSetInstance_ID() == 0)
					{
	...
	public void setTreatmentDocID(int hms_treatment_doc_ID)
	{
		if (hms_treatment_doc_ID < 1)
			set_Value("hms_treatment_doc_id", null);
		else
			set_Value("hms_treatment_doc_id", Integer.valueOf(hms_treatment_doc_ID));
	}

	public int getTreatmentDocID()
	{
		Integer ii = (Integer) get_Value("hms_treatment_doc_id");
		if (ii == null)
			return 0;
		return ii.intValue();
	}
