...
public class MMovement extends X_M_Movement implements DocAction
{
	...
	public MMovementLine[] getLines(boolean requery)
	{
		...
		List<MMovementLine> list = new Query(getCtx(), I_M_MovementLine.Table_Name, whereClause, get_TrxName())
				.setParameters(getM_Movement_ID()).setOrderBy(MMovementLine.COLUMNNAME_Line).list();
			...
			if (product != null && product.isStocked())
			{
				...
				if (line.getM_AttributeSetInstance_ID() == 0)
				{
					...
					for (int j = 0; j < mas.length; j++)
					{
						...
						// Update Storage
						if (!MStorage.add(getCtx(), locator.getM_Warehouse_ID(), line.getM_Locator_ID(),
								line.getM_Product_ID(), ma.getM_AttributeSetInstance_ID(), 0,
						...
						// Update Storage
						if (!MStorage.add(getCtx(), locator.getM_Warehouse_ID(), line.getM_LocatorTo_ID(),
								line.getM_Product_ID(), M_AttributeSetInstanceTo_ID, 0, ma.getMovementQty(), Env.ZERO,
						...
						trxFrom = new MTransaction(getCtx(), locator.getAD_Org_ID(),
								...
								ma.getM_AttributeSetInstance_ID(), ma.getMovementQty().negate(), getMovementDate(),
								get_TrxName());
						...
						MTransaction trxTo = new MTransaction(getCtx(), locatorTo.getAD_Org_ID(),
								...
								M_AttributeSetInstanceTo_ID, ma.getMovementQty(), getMovementDate(), get_TrxName());
				...
				if (trxFrom == null)
				{
					...
					// Update Storage
					if (!MStorage.add(getCtx(), locator.getM_Warehouse_ID(), line.getM_LocatorTo_ID(),
							line.getM_Product_ID(), line.getM_AttributeSetInstanceTo_ID(), 0, line.getMovementQty(),
							Env.ZERO, Env.ZERO, get_TrxName()))
					{
						m_processMsg = "Cannot correct Inventory (MA)";
						return DocAction.STATUS_Invalid;
					}
					...
					//
					trxFrom = new MTransaction(getCtx(), line.getAD_Org_ID(), MTransaction.MOVEMENTTYPE_MovementFrom,
	...
	private void updateWard(MMovement mMovement)
	{
		if (mMovement.getMovementtype() != null)
		{
			if (mMovement.getMovementtype().equalsIgnoreCase("ward"))
			{
				String sql = "SELECT * FROM hms_inpatient_orders WHERE m_movement_id =" + mMovement.get_ID();
				...
				try
				{
					...
					while (rs.next())
					{
						MInpatient_Orders io = new MInpatient_Orders(mMovement.getCtx(), rs, null);
						io.setorder_recieved(true);
						io.save();
					}
					rs.close();
					pstmt.close();
	...
	private void checkMaterialPolicy(MMovementLine line)
	{
		...
		if (line.getM_AttributeSetInstance_ID() == 0)
		{
			...
			MStorage[] storages = MStorage.getWarehouse(getCtx(), 0, line.getM_Product_ID(), 0, null,
	...
	{
		set_Value("movement_type", moveType);
	}

	public String getMovementtype()
	{
		return (String) get_Value("movement_type");
	}
} // MMovement
