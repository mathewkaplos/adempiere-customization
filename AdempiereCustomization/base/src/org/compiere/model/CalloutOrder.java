 ...
 *         <li>BF3468458 - Attribute Set Instance not filled on Orders when
 *         product lookup not used. See
 *         https://sourceforge.net/tracker/?func=detail&aid=3468458&group_id=176962&atid=879332
 */
public class CalloutOrder extends CalloutEngine
{
	...
	/**
	 * Order Header Change - DocType. - InvoiceRule/DeliveryRule/PaymentRule -
	...
	public String docType(Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value)
	{
		...
		try
		{
			...
			if (rs.next()) // we found document type
			{
				...
				// Invoice Rule
				if (DocSubTypeSO.equals(MOrder.DocSubTypeSO_POS) || DocSubTypeSO.equals(MOrder.DocSubTypeSO_Prepay)
			...
			if (DocSubTypeSO.equals(MOrder.DocSubTypeSO_POS) || DocSubTypeSO.equals(MOrder.DocSubTypeSO_Prepay)) // not
																													// for
																													// POS/PrePay
				;
			else
			{
				...
				if (rs.next())
				{
					...
					{
						if (IsSOTrx && (s.equals("B") || s.equals("S") || s.equals("U"))) // No
																							// Cash/Check/Transfer
																							// for
																							// SO_Trx
							s = "P"; // Payment Term
					...
					if (CreditLimit != 0)
					{
						...
						if (!rs.wasNull() && CreditAvailable < 0)
							mTab.fireDataStatusEEvent("CreditLimitOver",
									DisplayType.getNumberFormat(DisplayType.Amount).format(CreditAvailable), false);
					}
				}
	...
	public String bPartnerBill(Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value)
	{
		...
		String sql = "SELECT p.AD_Language,p.C_PaymentTerm_ID," + "p.M_PriceList_ID,p.PaymentRule,p.POReference,"
				+ "p.SO_Description,p.IsDiscountPrinted,"
				+ "p.InvoiceRule,p.DeliveryRule,p.FreightCostRule,DeliveryViaRule,"
				+ "p.SO_CreditLimit, p.SO_CreditLimit-p.SO_CreditUsed AS CreditAvailable," + "c.AD_User_ID,"
				+ "p.PO_PriceList_ID, p.PaymentRulePO, p.PO_PaymentTerm_ID,"
				+ "lbill.C_BPartner_Location_ID AS Bill_Location_ID " + "FROM C_BPartner p"
				+ " LEFT OUTER JOIN C_BPartner_Location lbill ON (p.C_BPartner_ID=lbill.C_BPartner_ID AND lbill.IsBillTo='Y' AND lbill.IsActive='Y')"
				+ " LEFT OUTER JOIN AD_User c ON (p.C_BPartner_ID=c.C_BPartner_ID) "
				+ "WHERE p.C_BPartner_ID=? AND p.IsActive='Y'"; // #1
	...
	public String priceList(Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value)
	{
		...
		String sql = "SELECT pl.IsTaxIncluded,pl.EnforcePriceLimit,pl.C_Currency_ID,c.StdPrecision,"
				...
				+ "FROM M_PriceList pl,C_Currency c,M_PriceList_Version plv " + "WHERE pl.C_Currency_ID=c.C_Currency_ID"
				+ " AND pl.M_PriceList_ID=plv.M_PriceList_ID" + " AND pl.M_PriceList_ID=? " // 1
	...
	public String product(Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value)
	{
		...
		{
			// If the product information is deleted, zero the other items as
			// well
		...
		//
		System.out.println("asi.getM_AttributeSetInstance_ID()::" + asi.getM_AttributeSetInstance_ID());
		//
		...
		if (Env.isSOTrx(ctx, WindowNo))
		{
			if (product.isStocked())
			{
				...
				else
				{
					...
					BigDecimal notReserved = MOrderLine.getNotReserved(ctx, M_Warehouse_ID, M_Product_ID,
							M_AttributeSetInstance_ID, C_OrderLine_ID.intValue());
					if (notReserved == null)
					...
					if (total.compareTo(QtyOrdered) < 0)
					{
						String info = Msg.parseTranslation(ctx,
								"@QtyAvailable@=" + available + " - @QtyNotReserved@=" + notReserved + " = " + total);
	...
	public String tax(Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value)
	{
		...
		//
		int C_Tax_ID = Tax.get(ctx, M_Product_ID, C_Charge_ID, billDate, shipDate, AD_Org_ID, M_Warehouse_ID,
				billC_BPartner_Location_ID, shipC_BPartner_Location_ID,
	...
	public String amt(Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value)
	{
		...
		if (mTab.getAD_Tab_ID() == 293 || mTab.getAD_Tab_ID() == 187 || mTab.getAD_Tab_ID()==1000150)
		{
			amt2(ctx, WindowNo, mTab, mField, value);
		} else
		{
			...
			BigDecimal QtyEntered, QtyOrdered, PriceEntered, PriceActual, PriceLimit, Discount, PriceList,
					discount_amount, rounded_lineamt;

			// Rounding Up
			BiFunction<BigDecimal, BigDecimal, BigDecimal> f1 = (num, div) -> div.compareTo(new BigDecimal(1)) == 0
					? num
					: div.multiply(((num.divide(div)).setScale(0, RoundingMode.CEILING)));

			// order line
			int C_OrderLine_ID = Env.getContextAsInt(ctx, WindowNo, "C_OrderLine_ID");
			MOrderLine line = new MOrderLine(ctx, C_OrderLine_ID, trxName);
			BigDecimal roudingTo = line.getRoundingTo();

			System.out.println(mTab.getValue("rounded_lineamt"));
			...
			discount_amount = (BigDecimal) mTab.getValue("discount_amount");
			rounded_lineamt = (BigDecimal) mTab.getValue("rounded_lineamt");
					...
					&& !"N".equals(Env.getContext(ctx, WindowNo, "DiscountSchema")))
			{
				...
				Env.setContext(ctx, WindowNo, "DiscountSchema", pp.isDiscountSchema() ? "Y" : "N");
				// mathew
				discount_amount = new BigDecimal(
						(PriceList.doubleValue() - PriceActual.doubleValue()) * QtyEntered.doubleValue());
				mTab.setValue("discount_amount", discount_amount);
			...
			} else if (mField.getColumnName().equals("PriceActual"))
			{
				...
				BigDecimal roundedLineamt = f1
						.apply(new BigDecimal(QtyEntered.doubleValue() * PriceActual.doubleValue()), roudingTo);
				mTab.setValue("rounded_lineamt", roundedLineamt);
				// line.save();
			...
			if (mField.getColumnName().equals("Discount"))
			{
				if (PriceList.doubleValue() != 0)
				{
					...
					discount_amount = new BigDecimal(
							(Discount.doubleValue() * QtyOrdered.doubleValue() * PriceList.doubleValue()) / 100.0);
					// discount_amount = new BigDecimal((Discount.doubleValue())
					// /
			...
			// Discount entered - Calculate Actual/Entered
			if (mField.getColumnName().equals("discount_amount"))
			{
				System.out.println("rounding_to:" + mTab.getValue("rounding_to"));
				if (PriceList.doubleValue() != 0)
				{
					Discount = new BigDecimal(
							((discount_amount.doubleValue()) / (PriceList.doubleValue()) * QtyOrdered.doubleValue())
									* 100);
					// PriceActual =
					// (((QtyOrdered.multiply(PriceActual)).subtract(discount_amount))
					// .divide(QtyOrdered.multiply(PriceActual)).multiply(PriceActual));

					PriceActual = new BigDecimal(
							(((QtyOrdered.doubleValue() * PriceList.doubleValue()) - discount_amount.doubleValue())
									/ (QtyOrdered.doubleValue() * PriceList.doubleValue())) * PriceList.doubleValue());
					// PriceActual = new BigDecimal((100.0 -
			...
			GridTab pTab = mTab.getParentTab();

			doSomething ds = () -> {
				pTab.setValue("PayAmt", Env.ZERO);
				pTab.setValue("DiscountAmt", Env.ZERO);
				pTab.setValue("tender_amt", Env.ZERO);
				pTab.setValue("Balance", Env.ZERO);
				pTab.setValue("RoundedPayAmt", Env.ZERO);
				pTab.dataSave(true);
				pTab.dataRefresh(true);
				pTab.dataSave(true);
			};
			...
			if (mField.getColumnName().equals("QtyEntered"))
			{
				...
				BigDecimal roundedLineamt = f1.apply(new BigDecimal(val.doubleValue() * PriceActual.doubleValue()),
						roudingTo);
				mTab.setValue("rounded_lineamt", roundedLineamt);
				mTab.dataSave(true);
				ds.execute();
	...
	public String amt2(Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value)
	{
		if (isCalloutActive() || value == null)
			return "";
				...
				&& !"N".equals(Env.getContext(ctx, WindowNo, "DiscountSchema")))
		{
			...
			if (PriceEntered == null)
				PriceEntered = pp.getPriceStd();
			//
			log.fine("QtyChanged -> PriceActual=" + pp.getPriceStd() + ", PriceEntered=" + PriceEntered + ", Discount="
					+ pp.getDiscount());
			PriceActual = pp.getPriceStd();
			mTab.setValue("PriceActual", pp.getPriceStd());
			mTab.setValue("Discount", pp.getDiscount());
	...
	public String qty(Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value)
	{
		...
		else if (mField.getColumnName().equals("QtyEntered"))
		{
			...
			if (QtyOrdered == null)
				QtyOrdered = QtyEntered;
			boolean conversion = QtyEntered.compareTo(QtyOrdered) != 0;
			log.fine("UOM=" + C_UOM_To_ID + ", QtyEntered=" + QtyEntered + " -> " + conversion + " QtyOrdered="
					+ QtyOrdered);
			Env.setContext(ctx, WindowNo, "UOMConversion", conversion ? "Y" : "N");
			mTab.setValue("QtyOrdered", QtyOrdered);
		}
		...
		else if (mField.getColumnName().equals("QtyOrdered"))
		{
			...
			log.fine("UOM=" + C_UOM_To_ID + ", QtyOrdered=" + QtyOrdered + " -> " + conversion + " QtyEntered="
					+ QtyEntered);
			Env.setContext(ctx, WindowNo, "UOMConversion", conversion ? "Y" : "N");
			mTab.setValue("QtyEntered", QtyEntered);
		} else
		...
		// Storage
		if (M_Product_ID != 0 && Env.isSOTrx(ctx, WindowNo) && QtyOrdered.signum() > 0) // no
																						// negative
																						// (returns)
		{
			...
			if (product.isStocked())
			{
				...
				BigDecimal available = MStorage.getQtyAvailable(M_Warehouse_ID, M_Product_ID, M_AttributeSetInstance_ID,
						null);
				if (available == null)
					available = Env.ZERO;
				if (available.signum() == 0)
				...
				else
				{
					...
					if (total.compareTo(QtyOrdered) < 0)
					{
						String info = Msg.parseTranslation(ctx, "@QtyAvailable@=" + available + "  -  @QtyNotReserved@="
								+ notReserved + "  =  " + total);
						mTab.fireDataStatusEEvent("InsufficientQtyAvailable", info, false);
					}
				}
