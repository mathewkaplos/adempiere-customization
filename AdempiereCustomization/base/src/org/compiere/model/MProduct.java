 ...
 *         <li>BF [ 1885414 ] ASI should be always mandatory if CostingLevel is
 *         Batch/Lot
 *         <li>FR [ 2093551 ] Refactor/Add
 *         org.compiere.model.MProduct.getCostingLevel
 *         <li>FR [ 2093569 ] Refactor/Add
 *         org.compiere.model.MProduct.getCostingMethod
 ...
 *         <li>BF [ 2814628 ] Wrong evaluation of Product inactive in
 *         beforeSave()
 */
public class MProduct extends X_M_Product
{
	...
	{
		List<MProduct> list = new Query(ctx, Table_Name, whereClause, trxName).setClient_ID().list();
	 ...
	 *             {@link #forS_Resource_ID(Properties, int, String)}
	 */
	...
	public static MProduct forS_Resource_ID(Properties ctx, int S_Resource_ID, String trxName)
	{
		...
		MProduct p = new Query(ctx, Table_Name, COLUMNNAME_S_Resource_ID + "=?", trxName)
				.setParameters(new Object[] { S_Resource_ID }).firstOnly();
	...
	/** Cache */
	private static CCache<Integer, MProduct> s_cache = new CCache<Integer, MProduct>(Table_Name, 40, 5); // 5
																											// minutes
	...
	public MProductDownload[] getProductDownloads(boolean requery)
	{
		if (m_downloads != null && !requery)
			return m_downloads;
		//
		List<MProductDownload> list = new Query(getCtx(), I_M_ProductDownload.Table_Name, "M_Product_ID=?",
				get_TrxName()).setOnlyActiveRecords(true).setOrderBy(I_M_ProductDownload.COLUMNNAME_Name)
	...
	protected boolean beforeSave(boolean newRecord)
	{
		...
		// it checks if UOM has been changed , if so disallow the change if the
		// condition is true.
		...
		{
			MAttributeSetInstance asi = new MAttributeSetInstance(getCtx(), getM_AttributeSetInstance_ID(),
					get_TrxName());
	...
	protected boolean hasInventoryOrCost()
	{
		...
		boolean hasTrx = new Query(getCtx(), MTransaction.Table_Name, MTransaction.COLUMNNAME_M_Product_ID + "=?",
				get_TrxName()).setOnlyActiveRecords(true).setParameters(new Object[] { get_ID() }).match();
		if (hasTrx)
		{
			return true;
		}
		...
		boolean hasCosts = new Query(getCtx(), I_M_CostDetail.Table_Name, I_M_CostDetail.COLUMNNAME_M_Product_ID + "=?",
				get_TrxName()).setOnlyActiveRecords(true).setParameters(get_ID()).match();
		if (hasCosts)
		{
			return true;
		}
	...
	protected boolean afterSave(boolean newRecord, boolean success)
	{
		...
		{
			String sql = "UPDATE A_Asset a " + "SET (Name, Description)="
					+ "(SELECT SUBSTR((SELECT bp.Name FROM C_BPartner bp WHERE bp.C_BPartner_ID=a.C_BPartner_ID) || ' - ' || p.Name,1,60), p.Description "
					+ "FROM M_Product p " + "WHERE p.M_Product_ID=a.M_Product_ID) " + "WHERE IsActive='Y'"
	...
	protected boolean beforeDelete()
	{
		 ...
		 * get_TrxName()); for(int i=0; i<mass.length; i++) { // Get Cost
		 * Elements MCostElement[] ces =
		 * MCostElement.getMaterialWithCostingMethods(this); MCostElement ce =
		 * null; for(int j=0; j<ces.length; j++) {
		 * if(MCostElement.COSTINGMETHOD_StandardCosting.equals(ces[i].
		 * getCostingMethod())) { ce = ces[i]; break; } }
		 * 
		 * if(ce == null) continue;
		 * 
		 * MCost mcost = MCost.get(this, 0, mass[i], 0,
		 * ce.getM_CostElement_ID()); mcost.delete(true, get_TrxName()); }
		 */

		// @Trifon Delete Product UOM Conversion
		final String whereClause = MProduct.COLUMNNAME_M_Product_ID + "=?";
		List<MUOMConversion> conversions = new Query(getCtx(), I_C_UOM_Conversion.Table_Name, whereClause,
				get_TrxName()).setClient_ID().setParameters(get_ID()).setOnlyActiveRecords(false).list();
		for (MUOMConversion conversion : conversions)
		{
			conversion.deleteEx(true);
		}
		// @Trifon Delete Product Downloads
		List<MProductDownload> downloads = new Query(getCtx(), I_M_ProductDownload.Table_Name, whereClause,
				get_TrxName()).setClient_ID().setParameters(get_ID()).setOnlyActiveRecords(false).list();
	...
	public MAttributeInstance getAttributeInstance(String name, String trxName)
	{
		...
		instance = (MAttributeInstance) table.getPO(
				MAttributeInstance.COLUMNNAME_M_AttributeSetInstance_ID + "=?" + " and "
						+ MAttributeInstance.COLUMNNAME_M_Attribute_ID + "=?",
				new Object[] { getM_AttributeSetInstance_ID(), attribute.getM_Attribute_ID() }, trxName);
		return instance;
	}
	 ...
	 */
	/*
	 * public boolean isASIMandatory(boolean isSOTrx) { // // If CostingLevel is
	 * BatchLot ASI is always mandatory - check all client acct schemas
	 * MAcctSchema[] mass = MAcctSchema.getClientAcctSchema(getCtx(),
	 * getAD_Client_ID(), get_TrxName()); for (MAcctSchema as : mass) { String
	 * cl = getCostingLevel(as); if
	 * (MAcctSchema.COSTINGLEVEL_BatchLot.equals(cl)) { return true; } } // //
	 * Check Attribute Set settings int M_AttributeSet_ID =
	 * getM_AttributeSet_ID(); if (M_AttributeSet_ID != 0) { MAttributeSet mas =
	 * MAttributeSet.get(getCtx(), M_AttributeSet_ID); if (mas == null ||
	 * !mas.isInstanceAttribute()) return false; // Outgoing transaction else if
	 * (isSOTrx) return mas.isMandatory(); // Incoming transaction else //
	 * isSOTrx == false return mas.isMandatoryAlways(); } // // Default not
	 * mandatory return false; }
	 */
	...
	{
		MProductCategoryAcct pca = MProductCategoryAcct.get(getCtx(), getM_Product_Category_ID(), as.get_ID(),
				get_TrxName());
	...
	{
		MProductCategoryAcct pca = MProductCategoryAcct.get(getCtx(), getM_Product_Category_ID(),
				accountSchema.get_ID(), get_TrxName());
	...
	{
		MProductCategoryAcct pca = MProductCategoryAcct.get(getCtx(), getM_Product_Category_ID(), assetSchema.get_ID(),
				AD_Org_ID, get_TrxName());
		String costingMethod = pca.getCostingMethod();
		if (costingMethod == null)
		{
	...
	public Integer getEnvAttributeSetInstance(Properties ctx, int WindowNo)
	{
		...
		} else if (getM_AttributeSet_ID() > 0 && M_AttributeSetInstance_ID > 0)
		{
			...
			// to use the same set.
			MAttributeSetInstance masi = MAttributeSetInstance.get(Env.getCtx(), M_AttributeSetInstance_ID,
	...
	public int getImport_product_id()
	{
		Integer ii = (Integer) get_Value("product_import_id");
		if (ii == null)
			return 0;
		return ii.intValue();
	}

	public void setImport_product_id(int product_import_id)
	{
		if (product_import_id < 1)
			set_Value("product_import_id", null);
		else
			set_Value("product_import_id", Integer.valueOf(product_import_id));
	}

	//
	public int getImport_category_id()
	{
		Integer ii = (Integer) get_Value("category_import_id");
		if (ii == null)
			return 0;
		return ii.intValue();
	}

	public void setImport_category_id(int category_import_id)
	{
		if (category_import_id < 1)
			set_Value("category_import_id", null);
		else
			set_Value("category_import_id", Integer.valueOf(category_import_id));
	}

	public void setInitialStock(BigDecimal initial_stock)
	{
		if (initial_stock == null)
			set_Value("initial_stock", Env.ZERO);
		else
			set_Value("initial_stock", initial_stock);
	}

	public BigDecimal getInitialStock()
	{

		BigDecimal bd = (BigDecimal) get_Value("initial_stock");
		if (bd == null)
			return Env.ZERO;
		return bd;
	}
	 ...
	 * Exists
	 */
	public static boolean exists(int M_Product_ID)
	{
		boolean exist = false;
		String sql = "SELECT * FROM M_Product WHERE M_Product_ID = " + M_Product_ID;
