...
public class MProductCategory extends X_M_Product_Category
{
	...
	public static boolean isCategory(int M_Product_Category_ID, int M_Product_ID)
	{
		...
		if (category != null)
		{
			...
			//
			s_log.fine("M_Product_ID=" + M_Product_ID + "(" + category + ") in M_Product_Category_ID="
					+ M_Product_Category_ID + " - " + (category.intValue() == M_Product_Category_ID));
			return category.intValue() == M_Product_Category_ID;
		}
	...
	/** Categopry Cache */
	private static CCache<Integer, MProductCategory> s_cache = new CCache<Integer, MProductCategory>(
			"M_Product_Category", 20);
	/** Product Cache */
	...
	/**
	 * Recursive search for parent nodes - climbs the to the root. If there is a
	 * circle there is no root but it comes back to the start node.
	 * 
	...
	public static boolean exists(int M_Product_Category_ID)
	{
		boolean exist = false;
		String sql = "SELECT * FROM M_Product_Category WHERE M_Product_Category_ID = " + M_Product_Category_ID;
