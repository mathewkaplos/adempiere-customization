...
public class CalloutPayment extends CalloutEngine
{
	...
	public String order(Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value)
	{
		...
		if (ts == null)
			ts = new Timestamp(System.currentTimeMillis());
		//
		String sql = "SELECT COALESCE(Bill_BPartner_ID, C_BPartner_ID) as C_BPartner_ID " + ", C_Currency_ID "
				+ ", GrandTotal " + "FROM C_Order WHERE C_Order_ID=?"; // #1
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		try
		{
	...
	public String amounts(Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value, Object oldValue)
	{
		if (isCalloutActive()) // assuming it is resetting value
			return "";
		if (mTab.getAD_Tab_ID() != 1000016)
		{
			System.out.println("Am heree..............");
			...
			if (C_Invoice_ID != 0)
			{
				...
				String sql = "SELECT C_BPartner_ID,C_Currency_ID," // 1..2
						+ " invoiceOpen(C_Invoice_ID,?)," // 3 #1
						+ " invoiceDiscount(C_Invoice_ID,?,?), IsSOTrx " // 4..5
																			// #2/3
						+ "FROM C_Invoice WHERE C_Invoice_ID=?"; // #4
				PreparedStatement pstmt = null;
				ResultSet rs = null;
				try
				{
					...
					if (rs.next())
					{
						...
						InvoiceOpenAmt = rs.getBigDecimal(3); // Set Invoice
																// Open
						// Amount
			...
			log.fine("Pay=" + PayAmt + ", Discount=" + DiscountAmt + ", WriteOff=" + WriteOffAmt + ", OverUnderAmt="
					+ OverUnderAmt);

			// Get Currency Info
			int C_Currency_ID = ((Integer) mTab.getValue("C_Currency_ID")) == null
					? Env.getContextAsInt(ctx, "$C_Currency_ID")
					: ((Integer) mTab.getValue("C_Currency_ID"));
			// .intValue ();
			...
			{
				log.fine("InvCurrency=" + C_Currency_Invoice_ID + ", PayCurrency=" + C_Currency_ID + ", Date="
						+ ConvDate + ", Type=" + C_ConversionType_ID);
				...
				if (CurrencyRate == null || CurrencyRate.compareTo(Env.ZERO) == 0)
				{
					...
					if (C_Currency_Invoice_ID == 0)
						return ""; // no error message when no invoice is
									// selected
					return "NoCurrencyConversion";
				}
				//
				InvoiceOpenAmt = InvoiceOpenAmt.multiply(CurrencyRate).setScale(currency.getStdPrecision(),
			...
			} else
			{
				...
				// Reason: we must make the callout is called just when
				// docstatus is
				// draft
		...
		} else if (mTab.getAD_Tab_ID() == 1000016)// payment in cashier
		{
			// RoundedPayAmt
			// tender_amt
			// PayAmt
			...
			BigDecimal PayAmt = (BigDecimal) mTab.getValue("PayAmt");
			BigDecimal DiscountAmt = (BigDecimal) mTab.getValue("DiscountAmt");
			BigDecimal RoundedPayAmt = (BigDecimal) mTab.getValue("RoundedPayAmt");
			...
			BigDecimal amt_topay_rounded = (BigDecimal) mTab.getValue("amt_topay_rounded");

			// System.out.println(RoundedPayAmt.doubleValue());
			System.out.println("Am here..............Payment at the cashier");

			// Changed Column
			String colName = mField.getColumnName();
			if (colName.equals("tender_amt"))
			{
				if (tender_amt.compareTo(amt_topay_rounded) == 1)
				{

					mTab.setValue("RoundedPayAmt", amt_topay_rounded);
					Balance = new BigDecimal(tender_amt.doubleValue() - amt_topay_rounded.doubleValue());
					mTab.setValue("PayAmt", amt_topay);
					mTab.setValue("Balance", Balance);
				}
				if (tender_amt.compareTo(amt_topay_rounded) == 0)
				{
				...
				if (tender_amt.compareTo(amt_topay_rounded) == -1)
				{
					BigDecimal diff = new BigDecimal(amt_topay_rounded.doubleValue() - tender_amt.doubleValue());

					mTab.setValue("RoundedPayAmt",
							new BigDecimal(amt_topay_rounded.doubleValue() - diff.doubleValue()));
			...
			{
				if (RoundedPayAmt.doubleValue() != 0.0 && PayAmt.doubleValue() != 0.0)
				{
					BigDecimal d_value = (BigDecimal) value;
					mTab.setValue("RoundedPayAmt", amt_topay_rounded.subtract(d_value));
