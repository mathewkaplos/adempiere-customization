...
public final class MLookup extends Lookup implements Serializable
{
	...
	public MLookup(MLookupInfo info, int TabNo)
	{
		...
		if (m_info.IsParent || m_info.IsKey)
		{
			...
			return; // required when parent needs to be selected (e.g. price
					// from product)
		}
		//
	...
	public NamePair get(Object key)
	{
		...
		{
			log.finer(
					(m_info.KeyColumn == null ? "ID=" + m_info.Column_ID : m_info.KeyColumn) + ": waiting for Loader");
			loadComplete();
			// is most current
		...
		log.finest(m_info.KeyColumn + ": " + key + "; Size=" + m_lookup.size() + "; Validated=" + m_info.IsValidated
				+ "; All Loaded=" + m_allLoaded + "; HasInactive=" + m_hasInactive);
		// never loaded
		if (!m_allLoaded && m_lookup.size() == 0 && !m_info.IsCreadedUpdatedBy && !m_info.IsParent
	...
	{
		return "MLookup[" + m_info.KeyColumn + ",Column_ID=" + m_info.Column_ID + ",Size=" + m_lookup.size()
				+ ",Validated=" + isValidated() + "-" + getValidation() + "]";
	} // toString
	...
	private ArrayList<Object> getData(boolean onlyValidated, boolean loadParent)
	{
		if (m_loader != null && m_loader.isAlive())
		{
			log.fine((m_info.KeyColumn == null ? "ID=" + m_info.Column_ID : m_info.KeyColumn) + ": waiting for Loader");
			loadComplete();
		}
	...
	public NamePair getDirect(Object key, boolean saveInCache, boolean cacheLocal)
	{
		...
		try
		{
			...
			if (rs.next())
			{
				...
				if (rs.next())
					log.log(Level.SEVERE, m_info.KeyColumn + ": Not unique (first returned) for " + key + " SQL="
							+ m_info.QueryDirect);
			} else
			{
				m_directNullKey = key;
	...
	private boolean isValidated(MLookupInfo info)
	{
		...
		if (validation.equals(info.parsedValidationCode))
			return true;
		return false;
	}
		...
		public void run()
		{
			...
			if (!m_info.IsValidated)
			{
				...
				if (validation.length() == 0 && m_info.ValidationCode.length() > 0)
				{
					...
					// globalqss - when called from Viewer window ignore error
					// about not parseable context variables
					// there is no context in report viewer windows
					boolean isReportViewer = Env.getContext(m_info.ctx, m_info.WindowNo, "_WinInfo_IsReportViewer")
							.equals("Y");
					if (!isReportViewer)
					{
						m_lookup.clear();
				...
				} else
				{
					...
					if (posOrder != -1)
						sql = sql.substring(0, posOrder) + (hasWhere ? " AND " : " WHERE ") + validation
			...
			// + " ID=" + m_info.AD_Column_ID + " " +
					+ " - Loader complete #" + size + " - all=" + m_allLoaded + " - ms="
