...
public class MProductPrice extends X_M_ProductPrice
{
	...
	{
		final String whereClause = MProductPrice.COLUMNNAME_M_PriceList_Version_ID + "=? AND "
		...
		MProductPrice retValue = new Query(ctx, I_M_ProductPrice.Table_Name, whereClause, trxName)
				.setParameters(M_PriceList_Version_ID, M_Product_ID).first();
		return retValue;
	} // get
	 ...
	 *            transaction
	 */
	public MProductPrice(Properties ctx, int M_PriceList_Version_ID, int M_Product_ID, BigDecimal PriceList,
	 ...
	 *            limit price
	 */
	public MProductPrice(MPriceListVersion plv, int M_Product_ID, BigDecimal PriceList, BigDecimal PriceStd,
	...
	{
		setPriceLimit(PriceLimit.setScale(this.getM_PriceList_Version().getM_PriceList().getPricePrecision(),
				BigDecimal.ROUND_HALF_UP));
		setPriceList(PriceList.setScale(this.getM_PriceList_Version().getM_PriceList().getPricePrecision(),
				BigDecimal.ROUND_HALF_UP));
		setPriceStd(PriceStd.setScale(this.getM_PriceList_Version().getM_PriceList().getPricePrecision(),
				BigDecimal.ROUND_HALF_UP));
	} // setPrice
	...
	public String toString()
	{
		...
		sb.append(getM_PriceList_Version_ID()).append(",M_Product_ID=").append(getM_Product_ID()).append(",PriceList=")
				.append(getPriceList()).append("]");
		return sb.toString();
	} // toString
	...
	protected boolean afterSave123(boolean newRecord, boolean success)
	{
		...
		if (!newRecord && (is_ValueChanged(COLUMNNAME_PriceList)))
		{
			BigDecimal price = this.getPriceList();
			this.setPriceLimit(price);
			this.setPriceStd(price);
			this.save();
		}
		return success;
	}
