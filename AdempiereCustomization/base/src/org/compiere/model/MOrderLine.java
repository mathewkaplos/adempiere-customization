 ...
 *         <li>BF [ 1733602 ] Price List including Tax Error - when a user
 *         changes the orderline or invoice line for a product on a price list
 *         that includes tax, the net amount is incorrectly calculated.
 */
public class MOrderLine extends X_C_OrderLine
{
	 ...
	 */
	public static BigDecimal getNotReserved(Properties ctx, int M_Warehouse_ID, int M_Product_ID,
			int M_AttributeSetInstance_ID, int excludeC_OrderLine_ID)
	{
		BigDecimal retValue = Env.ZERO;
		String sql = "SELECT SUM(QtyOrdered-QtyDelivered-QtyReserved) " + "FROM C_OrderLine ol"
				+ " INNER JOIN C_Order o ON (ol.C_Order_ID=o.C_Order_ID) " + "WHERE ol.M_Warehouse_ID=?" // #1
				+ " AND M_Product_ID=?" // #2
				+ " AND o.IsSOTrx='Y' AND o.DocStatus='DR'" + " AND QtyOrdered-QtyDelivered-QtyReserved<>0"
	...
	/**
	 * Set Price for Product and PriceList. Use only if newly created. Uses
	 * standard price list of not set by order constructor
	 */
	...
	public void setPrice(int M_PriceList_ID)
	{
		...
		if (getQtyEntered().compareTo(getQtyOrdered()) == 0)
			...
			setPriceEntered(
					getPriceActual().multiply(getQtyOrdered().divide(getQtyEntered(), 12, BigDecimal.ROUND_HALF_UP))); // recision
	...
	public boolean setTax()
	{
		int ii = Tax.get(getCtx(), getM_Product_ID(), getC_Charge_ID(), getDateOrdered(), getDateOrdered(),
				getAD_Org_ID(), getM_Warehouse_ID(), getC_BPartner_Location_ID(), // should
																					// be
																					// bill
																					// to
	...
	public void setLineNetAmt()
	{
		...
		// juddm: Tax Exempt & Tax Included in Price List & not Document Level -
		// Adjust Line Amount
		...
		if (isTaxIncluded() && !documentLevel)
		{
			...
			if (getProduct() == null)
			{
				...
				stdTax = new MTax(getCtx(),
						((MTaxCategory) getProduct().getC_TaxCategory()).getDefaultTax().getC_Tax_ID(), get_TrxName());
			...
			if (stdTax != null)
			{
				...
				log.fine("Price List includes Tax and Tax Changed on Order Line: New Tax Amt: " + taxThisAmt
						+ " Standard Tax Amt: " + taxStdAmt + " Line Net Amt: " + bd);
			}
	...
	{
		StringBuffer sb = new StringBuffer("MOrderLine[").append(get_ID()).append(", Line=").append(getLine())
				.append(", Ordered=").append(getQtyOrdered()).append(", Delivered=").append(getQtyDelivered())
				.append(", Invoiced=").append(getQtyInvoiced()).append(", Reserved=").append(getQtyReserved())
				.append(", LineNet=").append(getLineNetAmt()).append("]");
		return sb.toString();
	} // toString
	...
	public void setDiscount()
	{
		...
		if (Env.ZERO.compareTo(list) == 0)
			return;
		BigDecimal discount = list.subtract(getPriceActual()).multiply(new BigDecimal(100)).divide(list, getPrecision(),
	...
	public boolean isTaxIncluded()
	{
		if (m_M_PriceList_ID == 0)
		{
			m_M_PriceList_ID = DB.getSQLValue(get_TrxName(), "SELECT M_PriceList_ID FROM C_Order WHERE C_Order_ID=?",
					getC_Order_ID());
		}
	...
	protected boolean beforeSave123(boolean newRecord)
	{
		...
		if (getC_BPartner_ID() == 0 || getC_BPartner_Location_ID() == 0 || getM_Warehouse_ID() == 0
				|| getC_Currency_ID() == 0)
			setOrder(getParent());
		if (m_M_PriceList_ID == 0)
		...
		else // Set/check Product Price
		{
			...
			if (m_productPrice == null && Env.ZERO.compareTo(getPriceActual()) == 0
		...
		// UOM
		if (getC_UOM_ID() == 0
				&& (getM_Product_ID() != 0 || getPriceEntered().compareTo(Env.ZERO) != 0 || getC_Charge_ID() != 0))
		{
				...
				|| is_ValueChanged("M_AttributeSetInstance_ID") || is_ValueChanged("M_Warehouse_ID")))
		{
			...
			if (product.isStocked())
			{
				...
				if (isInstance)
				{
					MStorage[] storages = MStorage.getWarehouse(getCtx(), getM_Warehouse_ID(), getM_Product_ID(),
							getM_AttributeSetInstance_ID(), M_AttributeSet_ID, false, null, true, get_TrxName());
	...
	private boolean updateHeaderTax()
	{
		...
		// Update Order Header
		String sql = "UPDATE C_Order i" + " SET TotalLines="
		...
		if (isTaxIncluded())
			...
			sql = "UPDATE C_Order i " + " SET GrandTotal=TotalLines+"
	...
	/**
	 * @author- mathew 18.05.17 Item Types CON -Consultation PRO- Product LAB-
	 *          Laboratory Service
	 */
	public void setitem_type(String itemtype)
	{
		int no = 0;
		String sql = "UPDATE C_Orderline SET item_type ='" + itemtype + "' WHERE C_Orderline_ID =" + get_ID();
		no = DB.executeUpdate(sql, get_TrxName());
		// System.out.println(no + " orderlines updated");
	}

	public String getitem_type()
	{
		String retValue = null;
		String sql = "SELECT item_type FROM C_Orderline WHERE C_Orderline_ID=" + get_ID();
		PreparedStatement pstmt = null;
		try
		{
			pstmt = DB.prepareStatement(sql, null);
	...
	public void setIspaid(String b)
	{
		int no = 0;
		String sql = "UPDATE C_Orderline SET paid ='" + b + "' WHERE C_Orderline_ID =" + get_ID();
		no = DB.executeUpdate(sql, get_TrxName());
	}

	public void setIspay(String b)
	{
		int no = 0;
		String sql = "UPDATE C_Orderline SET pay ='" + b + "' WHERE C_Orderline_ID =" + get_ID();
		no = DB.executeUpdate(sql, get_TrxName());
	}

	public void setc_orderline_id_source(int orderline_id_source)
	{
		int no = 0;
		String sql = "UPDATE C_Orderline SET orderline_id_source ='" + orderline_id_source + "' WHERE C_Orderline_ID ="
				+ get_ID();
		no = DB.executeUpdate(sql, get_TrxName());
	}
	...
	public String getXX()
	{
		return (String) get_Value("item_type");
	}
	 ...
	 * Set rounded_lineamt .
	 * 
	 * @param rounded_lineamt
	 *            rounded_lineamt in percent
	 */
	public void setRoundedLineamt(BigDecimal rounded_lineamt)
	{
		set_Value("rounded_lineamt", rounded_lineamt);
	}

	/**
	 * Get RoundedLineamt.
	 * 
	 * @return RoundedLineamt
	 */
	public BigDecimal getRoundedLineamt()
	{
		BigDecimal bd = (BigDecimal) get_Value("rounded_lineamt");
		if (bd == null)
			return Env.ZERO;
		return bd;
	}
	 ...
	 * 
	 * @param rounding_to
	 *            rounding_to in percent
	 */
	public void setRoundingTo(BigDecimal rounding_to)
	{
		set_Value("rounding_to", rounding_to);
	}
	 ...
	 * @return rounding_to
	 */
	public BigDecimal getRoundingTo()
	{
		BigDecimal bd = (BigDecimal) get_Value("rounding_to");
		if (bd == null)
			return Env.ZERO;
		return bd;
	}
