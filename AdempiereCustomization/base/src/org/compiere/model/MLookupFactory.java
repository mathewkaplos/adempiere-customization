 ...
 *         <li>BF [ 1734394 ] MLookupFactory.getLookup_TableDirEmbed is not
 *         translated
 *         <li>BF [ 1714261 ] MLookupFactory: TableDirEmbed -> TableEmbed not
 *         supported
 *         <li>BF [ 1672820 ] Sorting should be language-sensitive
 *         <li>BF [ 1739530 ] getLookup_TableDirEmbed error when BaseColumn is
 *         sql query
 *         <li>BF [ 1739544 ] getLookup_TableEmbed error for self referencing
 *         references
...
public class MLookupFactory
{
	...
	private static CCache<String, MLookupInfo> s_cacheRefTable = new CCache<String, MLookupInfo>("AD_Ref_Table", 30,
			60); // 1h
	 ...
	 *            display type
	 * @param Column_ID
	 *            AD_Column_ID or AD_Process_Para_ID
	 * @param language
	 *            report language
	 * @param ColumnName
	 *            key column name
	 * @param AD_Reference_Value_ID
	 *            AD_Reference (List, Table)
	 * @param IsParent
	 *            parent (prevents query to directly access value)
	 ...
	 * @return MLookup
	 */
	public static MLookup get(Properties ctx, int WindowNo, int Column_ID, int AD_Reference_ID, Language language,
			String ColumnName, int AD_Reference_Value_ID, boolean IsParent, String ValidationCode) throws Exception
	{
		MLookupInfo info = getLookupInfo(ctx, WindowNo, Column_ID, AD_Reference_ID, language, ColumnName,
	...
	{
		//system.out.println(AD_Reference_ID);
		String ColumnName = "";
		int AD_Reference_Value_ID = 0;
		boolean IsParent = false;
		String ValidationCode = "";
		//
		String sql = "SELECT c.ColumnName, c.AD_Reference_Value_ID, c.IsParent, vr.Code " + "FROM AD_Column c"
				+ " LEFT OUTER JOIN AD_Val_Rule vr ON (c.AD_Val_Rule_ID=vr.AD_Val_Rule_ID) " + "WHERE c.AD_Column_ID=?";
		...
		//
		MLookupInfo info = getLookupInfo(ctx, WindowNo, Column_ID, AD_Reference_ID, Env.getLanguage(ctx), ColumnName,
				AD_Reference_Value_ID, IsParent, ValidationCode);
	...
	{
		//system.out.println("Hai!:" + AD_Reference_ID);
		//
	...
	/**************************************************************************
	 * Get Information for Lookups based on Column_ID for Table Columns or
	 * Process Parameters.
	 *
	 ...
	 */
	static public MLookupInfo getLookupInfo(Properties ctx, int WindowNo, int Column_ID, int AD_Reference_ID,
			Language language, String ColumnName, int AD_Reference_Value_ID, boolean IsParent, String ValidationCode)
	{
		//system.out.println("AD_Reference_ID: " + AD_Reference_ID);
		...
		{
			// String newSQL = Env.parseContext(ctx, WindowNo, info.Query,
			// false);
			String newSQL = Env.parseContext(ctx, 0, info.Query, false); // only
																			// global
			if (newSQL.length() == 0)
			{
		...
		if (posOrder == -1)
			info.QueryDirect = info.Query + (hasWhere ? " AND " : " WHERE ") + info.KeyColumn + "=?";
		else
			info.QueryDirect = info.Query.substring(0, posOrder) + (hasWhere ? " AND " : " WHERE ") + info.KeyColumn
					+ "=?";
		...
		{
			/*
			 * local_validationCode = Env.parseContext (ctx, WindowNo,
			 * info.ValidationCode, true); if (local_validationCode.length() ==
			 * 0) // returns "" if not all variables were parsed
			 * info.IsValidated = false; else info.IsValidated = true;
			 */
			info.IsValidated = false;
		}
		/*
		 * // Add Local Validation if (local_validationCode.length() != 0) {
		 * //jz handle no posOrder case if (posOrder > 0) info.Query =
		 * info.Query.substring(0, posOrder) + (hasWhere ? " AND " : " WHERE ")
		 * + local_validationCode + info.Query.substring(posOrder); else
		 * info.Query = info.Query + (hasWhere ? " AND " : " WHERE ") +
		 * local_validationCode; }
		 */

		// Add Security
		if (needToAddSecurity)
			info.Query = MRole.getDefault(ctx, false).addAccessSQL(info.Query, info.TableName, MRole.SQL_FULLYQUALIFIED,
					MRole.SQL_RO);
		//
	...
	{
		String byValue = DB.getSQLValueString(null,
				"SELECT IsOrderByValue FROM AD_Reference WHERE AD_Reference_ID = ? ", AD_Reference_Value_ID);
		...
		if (Env.isBaseLanguage(language, "AD_Ref_List"))
			...
			realSQL.append("trl.Name, AD_Ref_List.IsActive " + "FROM AD_Ref_List INNER JOIN AD_Ref_List_Trl trl "
		...
		if ("Y".equals(byValue))
			...
			realSQL.append(" ORDER BY 3"); // sort by name/translated name -
											// teo_sarca, [ 1672820 ]
		//
		return new MLookupInfo(realSQL.toString(), "AD_Ref_List", "AD_Ref_List.Value", 101, 101,
				MQuery.getEqualQuery("AD_Reference_ID", AD_Reference_Value_ID), false); // Zoom
																						// Window+Query
	} // getLookup_List
	 ...
	 *            link column name
	 * @return SELECT Name FROM AD_Ref_List WHERE AD_Reference_ID=x AND
	 *         Value=linkColumn
	 */
	static public String getLookup_ListEmbed(Language language, int AD_Reference_Value_ID, String linkColumnName)
	{
		...
		if (Env.isBaseLanguage(language, "AD_Ref_List"))
			...
			realSQL.append("trl.Name " + "FROM AD_Ref_List INNER JOIN AD_Ref_List_Trl trl "
					+ " ON (AD_Ref_List.AD_Ref_List_ID=trl.AD_Ref_List_ID AND trl.AD_Language='")
					.append(language.getAD_Language()).append("')");
		realSQL.append(" WHERE AD_Ref_List.AD_Reference_ID=").append(AD_Reference_Value_ID)
	 ...
	 * 
	 * @param ctx
	 *            context for access and dynamic access
	 * @param language
	 *            report language
	 * @param WindowNo
	 *            window no
	 * @param AD_Reference_Value_ID
	 *            reference value
	 * @return SELECT Key, NULL, Name, IsActive FROM Table - if KeyColumn end
	 *         with _ID otherwise SELECT NULL, Key, Name, IsActive FROM Table
	 */
	static private MLookupInfo getLookup_Table(Properties ctx, Language language, int WindowNo,
			int AD_Reference_Value_ID)
	{
		...
		// boolean isSOTrx = !"N".equals(Env.getContext(ctx, WindowNo,
		// "IsSOTrx"));
		int ZoomWindow = 0;
		int ZoomWindowPO = 0;
		...
		try
		{
			...
			if (rs.next())
			{
				...
				// isValueDisplayed = "Y".equals(rs.getString(4)); //MAthew
		...
		if (IsTranslated && !Env.isBaseLanguage(language, TableName))
		{
			...
			realSQL.append(" FROM ").append(TableName).append(" INNER JOIN ").append(TableName).append("_TRL ON (")
					.append(TableName).append(".").append(KeyColumn).append("=").append(TableName).append("_Trl.")
					.append(KeyColumn).append(" AND ").append(TableName).append("_Trl.AD_Language='")
					.append(language.getAD_Language()).append("')");
		}
		// Not Translated
		else
			...
			if (where.length() != 0)
			{
				...
				if (where.indexOf('.') == -1)
					s_log.log(Level.SEVERE,
							"getLookup_Table - " + TableName + ": WHERE should be fully qualified: " + WhereClause);
		...
		//system.out.println("...18 18 18." + realSQL);
		...
		retValue = new MLookupInfo(realSQL.toString(), TableName, TableName + "." + KeyColumn, ZoomWindow, ZoomWindowPO,
				zoomQuery, isAlert);
	 ...
	 */
	static public String getLookup_TableEmbed(Language language, String BaseColumn, String BaseTable,
			int AD_Reference_Value_ID)
	{
		String sql = "SELECT t.TableName,ck.ColumnName AS KeyColumn,"
				+ "cd.ColumnName AS DisplayColumn,rt.isValueDisplayed,cd.IsTranslated," + "rt.DisplaySQL "
				+ "FROM AD_Ref_Table rt" + " INNER JOIN AD_Table t ON (rt.AD_Table_ID=t.AD_Table_ID)"
				+ " INNER JOIN AD_Column ck ON (rt.AD_Key=ck.AD_Column_ID)"
				+ " INNER JOIN AD_Column cd ON (rt.AD_Display=cd.AD_Column_ID) " + "WHERE rt.AD_Reference_ID=?"
				+ " AND rt.IsActive='Y' AND t.IsActive='Y'";
	...
	static private MLookupInfo getLookup_TableDir(Properties ctx, Language language, int WindowNo, String ColumnName)
	{
		...
		String sql0 = "SELECT c.ColumnName,c.IsTranslated,c.AD_Reference_ID,"
				+ "c.AD_Reference_Value_ID,t.AD_Window_ID,t.PO_Window_ID " + ", c.ColumnSQL " // 7
				+ "FROM AD_Table t" + " INNER JOIN AD_Column c ON (t.AD_Table_ID=c.AD_Table_ID) " + "WHERE TableName=?"
				+ " AND c.IsIdentifier='Y' " + "ORDER BY c.SeqNo";
		//
		...
		try
		{
			...
			while (rs.next())
			{
				LookupDisplayColumn ldc = new LookupDisplayColumn(rs.getString(1), rs.getString(7), // ColumnSQL
						"Y".equals(rs.getString(2)), rs.getInt(3), rs.getInt(4));
				list.add(ldc);
				// s_log.fine("getLookup_TableDir: " + ColumnName + " - " +
				// ldc);
				//
		...
		MLookupInfo lInfo = new MLookupInfo(realSQL.toString(), TableName, TableName + "." + KeyColumn, ZoomWindow,
				ZoomWindowPO, zoomQuery, false);
	 ...
	 *            base column
	 * @return SELECT Column FROM TableName WHERE
	 *         BaseTable.BaseColumn=TableName.ColumnName
	 */
	static public String getLookup_TableDirEmbed(Language language, String ColumnName, String BaseTable,
			String BaseColumn)
	{
		...
		String sql = "SELECT c.ColumnName,c.IsTranslated,c.AD_Reference_ID,c.AD_Reference_Value_ID " + ", c.ColumnSQL " // 5
				+ "FROM AD_Table t INNER JOIN AD_Column c ON (t.AD_Table_ID=c.AD_Table_ID) " + "WHERE TableName=?"
				+ " AND c.IsIdentifier='Y' " + "ORDER BY c.SeqNo";
		//
		ArrayList<LookupDisplayColumn> list = new ArrayList<LookupDisplayColumn>();
		...
		try
		{
			...
			while (rs.next())
			{
				LookupDisplayColumn ldc = new LookupDisplayColumn(rs.getString(1), rs.getString(5),
						"Y".equals(rs.getString(2)), rs.getInt(3), rs.getInt(4));
				list.add(ldc);
				// s_log.fine("getLookup_TableDirEmbed: " + ColumnName + " - " +
				// ldc);
				//
				if (!isTranslated && ldc.IsTranslated)
					isTranslated = true;
		...
		if (isTranslated && !Env.isBaseLanguage(language, TableName))
		{
			embedSQL.append(" INNER JOIN ").append(TableName).append("_TRL ON (").append(TableName).append(".")
					.append(KeyColumn).append("=").append(TableName).append("_Trl.").append(KeyColumn).append(" AND ")
					.append(TableName).append("_Trl.AD_Language=").append(DB.TO_STRING(language.getAD_Language()))
					.append(")");
		}
		embedSQL.append(" WHERE ");
		// If is not virtual column - teo_sarca [ 1739530 ]
