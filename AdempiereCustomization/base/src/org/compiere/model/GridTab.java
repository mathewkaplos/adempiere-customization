...
/**
 * Tab Model. - a combination of AD_Tab (the display attributes) and AD_Table
 * information.
 * <p>
 * The Tab owns also it's Table model and listens to data changes to update the
 * Field values.
 *
 * <p>
 ...
 *         <li>BF [ 2104022 ] GridTab.processCallout: throws NPE if callout
 *         returns null
 *         <li>FR [ 2846871 ] Add method
 *         org.compiere.model.GridTab.getIncludedTabs
 ...
 *         <li>BF [ 2874109 ] Tab ORDER BY clause is not supporting context
 *         variables
 ...
 *         <li>BF [ 2910358 ] Error in context when a field is found in
 *         different tabs.
 *         https://sourceforge.net/tracker/?func=detail&aid=2910358&group_id=176962&atid=879332
 *         <li>BF [ 2910368 ] Error in context when IsActive field is found in
 *         different
 *         https://sourceforge.net/tracker/?func=detail&aid=2910368&group_id=176962&atid=879332
 * @author Carlos Ruiz, qss FR [1877902]
 * @see http://sourceforge.net/tracker/?func=detail&atid=879335&aid=1877902&group_id=176962
 *      to FR [1877902]
...
public class GridTab implements DataStatusListener, Evaluatee, Serializable
{
	...
	/**
	 * Get Field data and add to MTable, if it's required or displayed. Required
	 * fields are keys, parents, or standard Columns
	 * 
	...
	private boolean loadFields()
	{
		...
		for (int f = 0; f < m_vo.getFields().size(); f++)
		{
			...
			if (voF != null)
			{
				...
				/*
				 * if(this.isReadOnly()) { voF.IsReadOnly = true; }
				 */
				// Record Info
				if (field.isKey())
				{
		...
		if (!m_mTable.getTableName().equals(X_AD_PInstance_Log.Table_Name))
		{ // globalqss, bug 1662433
			// Add Standard Fields
			if (m_mTable.getField("Created") == null)
			{
				GridField created = new GridField(GridFieldVO.createStdField(m_vo.ctx, m_vo.WindowNo, m_vo.TabNo,
						m_vo.AD_Window_ID, m_vo.AD_Tab_ID, false, true, true));
				m_mTable.addField(created);
			}
			if (m_mTable.getField("CreatedBy") == null)
			{
				GridField createdBy = new GridField(GridFieldVO.createStdField(m_vo.ctx, m_vo.WindowNo, m_vo.TabNo,
						m_vo.AD_Window_ID, m_vo.AD_Tab_ID, false, true, false));
				m_mTable.addField(createdBy);
			}
			if (m_mTable.getField("Updated") == null)
			{
				GridField updated = new GridField(GridFieldVO.createStdField(m_vo.ctx, m_vo.WindowNo, m_vo.TabNo,
						m_vo.AD_Window_ID, m_vo.AD_Tab_ID, false, false, true));
				m_mTable.addField(updated);
			}
			if (m_mTable.getField("UpdatedBy") == null)
			{
				GridField updatedBy = new GridField(GridFieldVO.createStdField(m_vo.ctx, m_vo.WindowNo, m_vo.TabNo,
	...
	/**
	 * Get TableModel. <B>Do not directly communicate with the table model, but
	 * through the methods of this class</B>
	 * 
	 * @return Table Model
	 */
	 ...
	 *            if only current row, how many days back
	 * @param maxRows
	...
	public void query(boolean onlyCurrentRows, int onlyCurrentDays, int maxRows)
	{
		...
		log.fine("#" + m_vo.TabNo + " - Only Current Rows=" + onlyCurrentRows + ", Days=" + onlyCurrentDays
				+ ", Detail=" + isDetail());
		// Is it same query?
		boolean refresh = m_oldQuery.equals(m_query.getWhereClause()) && m_vo.onlyCurrentRows == onlyCurrentRows
			...
			} else
			{
				...
				if (m_parentColumnName.length() > 0)
				{
					...
					if (value == null || value.length() == 0)
						value = Env.getContext(m_vo.ctx, m_vo.WindowNo, m_parentColumnName, true); // back
																									// compatibility
				} else
				{
					value = Env.getContext(m_vo.ctx, m_vo.WindowNo, getParentTabNo(), lc, true);
					if (value == null || value.length() == 0)
						value = Env.getContext(m_vo.ctx, m_vo.WindowNo, lc, true); // back
																					// compatibility
				}

				// Same link value?
				if (!m_linkValue.equals(value))
				{ // We have a new parent tab. Wipe the query.
					m_query = new MQuery();
				}
				if (refresh)
				...
				if (value.length() == 0)
				{
					...
					// as opened by this fix [ 1881480 ] Navigation problem
					// between tabs
					// it's safer to avoid retrieving details at all if there is
					// no parent value
					where.append(" 2=3");
				} else
				{
					// if (!m_query.isActive()){ // create a where criteria -
	...
	private String validateQuery(MQuery query)
	{
		...
		String sql = "SELECT cc.ColumnName " + "FROM AD_Column c"
				+ " INNER JOIN AD_Ref_Table r ON (c.AD_Reference_Value_ID=r.AD_Reference_ID)"
				+ " INNER JOIN AD_Column cc ON (r.AD_Key=cc.AD_Column_ID) " + "WHERE c.AD_Reference_ID IN (18,30)" // Table/Search
				+ " AND c.ColumnName=?";
		try
		{
		...
		sql = "SELECT t.TableName " + "FROM AD_Column c" + " INNER JOIN AD_Table t ON (c.AD_Table_ID=t.AD_Table_ID) "
		...
		StringBuffer result = new StringBuffer(getTableName()).append(".").append(tabKeyColumn)
				.append(" IN (SELECT xx.").append(tabKeyColumn).append(" FROM ").append(tableName).append(" xx WHERE ")
				.append(query.getWhereClause(true)).append(")");
	 ...
	 * 
	 * @param manualCmd
	 *            if true, no vetoable PropertyChange will be fired for save
	 *            confirmation from MTable
		...
		try
		{
			...
			if (manualCmd)
			{
				...
				if (m_lastDataStatusEvent != null && m_lastDataStatusEvent.getCurrentRow() == m_currentRow
						&& ((m_lastDataStatusEvent.Record_ID != null
								&& m_lastDataStatusEvent.Record_ID instanceof Integer
								&& (Integer) m_lastDataStatusEvent.Record_ID == 0)
								|| m_lastDataStatusEvent.Record_ID == null))
				{
	...
	// Validate if the current tab record has changed in database or any parent
	// record
	// Return if there are changes
	public boolean hasChangedCurrentTabAndParents()
	{
//		String msg = null;
//		// Carlos Ruiz / globalqss - [ adempiere-Bugs-1985481 ] Processed
//		// documents can be edited
//		// Validate that current record has not changed and validate that every
//		// parent above has not changed
//		if (m_mTable.hasChanged(m_currentRow))
//		{
//			// return error stating that current record has changed and it
//			// cannot be saved
...
//		{
//			// get parent tab
//			// the parent tab is the first tab above with level =
//			// this_tab_level-1
...
//					{
//						// return error stating that current record has changed
...
//		}
	 ...
	 *            if true the value of a field was actually changed (e.g. for
	 *            new records, which have not been changed) - default false
	...
	public boolean dataNew(boolean copy)
	{
		...
		{
			// Allow New Where main Record is processed for
			// some specific tab - mathew-kipchumba 14.05.2017
			// Payment tab(cashier)=1000016
			// payment tab(invoice payment) =-1000088
			if (m_vo.AD_Tab_ID != 1000016 && m_vo.AD_Tab_ID != 1000088)
			{

				boolean processed = "Y".equals(Env.getContext(m_vo.ctx, m_vo.WindowNo, "Processed"));
				// boolean active = "Y".equals(Env.getContext(m_vo.ctx,
				// m_vo.WindowNo, "IsActive"));
				if (processed)
				{
		...
		/**
		 * temporary set currentrow to point to the new row to ensure even cause
		 * by m_mTable.dataNew is handle properly.
		 */
		...
		// process all Callouts (no dependency check - assumed that settings are
		// valid)
	...
	/**
	 * Set Name of link column. Set from MWindow.loadTabData Used in
	 * MTab.isCurreny, (.setCurrentRow) .query - APanel.cmd_report and
	 * MField.isEditable and .isDefault via context
	 * 
	 ...
	 * @param isIncluded
	 *            true if included
	 * @deprecated The method getIncluded now validate against the structure,
	...
	public boolean isDisplayed()
	{
		...
		// ** dynamic content **
		String parsed = Env.parseContext(m_vo.ctx, this.getWindowNo(), dl, false, false).trim(); // Add
																									// WindowNo
		if (parsed.length() == 0)
			return true;
	 ...
	 * @deprecated the functionality related to AD_Tab.Included_Tab_ID was not
	 *             developed
	 */
	...
	public String getTrxInfo()
	{
		...
		if (m_vo.TableName.startsWith("C_InvoiceBatch"))
		{
			...
			try
			{
				mf = new MessageFormat(Msg.getMsg(Env.getAD_Language(m_vo.ctx), "InvoiceBatchSummary"),
						Env.getLanguage(m_vo.ctx).getLocale());
			} catch (Exception e)
			{
				log.log(Level.SEVERE,
						"InvoiceBatchSummary=" + Msg.getMsg(Env.getAD_Language(m_vo.ctx), "InvoiceBatchSummary"), e);
			}
			if (mf == null)
				return " ";
			/**********************************************************************
			 * ** Message: ExpenseSummary ** {0} Line(s) {1,number,#,##0.00} -
			 * Total: {2,number,#,##0.00}
			 *
			 * {0} - Number of lines {1} - Total {2} - Currency
			 */
			Object[] arguments = new Object[3];
			boolean filled = false;
			//
			String sql = "SELECT COUNT(*), NVL(SUM(LineNetAmt),0), NVL(SUM(LineTotalAmt),0) "
					+ "FROM C_InvoiceBatchLine " + "WHERE C_InvoiceBatch_ID=? AND IsActive='Y'";
			//
			try
			{
				PreparedStatement pstmt = DB.prepareStatement(sql, null);
		...
		else if (m_vo.TableName.startsWith("C_Order") || m_vo.TableName.startsWith("C_Invoice"))
		{
			...
			if (isOrder)
			{
				...
				sql.append("FROM C_Order o" + " INNER JOIN C_Currency c ON (o.C_Currency_ID=c.C_Currency_ID)"
						+ " INNER JOIN C_OrderLine l ON (o.C_Order_ID=l.C_Order_ID) " + "WHERE o.C_Order_ID=? ");
			} else
			{
				Record_ID = Env.getContextAsInt(m_vo.ctx, m_vo.WindowNo, "C_Invoice_ID");
				sql.append("FROM C_Invoice o" + " INNER JOIN C_Currency c ON (o.C_Currency_ID=c.C_Currency_ID)"
			...
			if (mf == null)
				return " ";
			/**********************************************************************
			 * ** Message: OrderSummary ** {0} Line(s) - {1,number,#,##0.00} -
			 * Total: {2,number,#,##0.00} {3} = {4,number,#,##0.00}
			 *
			 * {0} - Number of lines {1} - Line total {2} - Grand total
			 * (including tax, etc.) {3} - Currency (4) - Grand total converted
			 * to local currency
			 */
		...
		else if (m_vo.TableName.startsWith("S_TimeExpense") && m_vo.TabNo == 0)
		{
			...
			{
				log.log(Level.SEVERE, "ExpenseSummary=" + Msg.getMsg(Env.getAD_Language(m_vo.ctx), "ExpenseSummary"),
						e);
			}
			if (mf == null)
				return " ";
			/**********************************************************************
			 * ** Message: ExpenseSummary ** {0} Line(s) - Total:
			 * {1,number,#,##0.00} {2}
			 *
			 * {0} - Number of lines {1} - Total {2} - Currency
			 */
			...
			boolean filled = false;
			//
			String SQL = "SELECT COUNT(*) AS Lines, SUM(ConvertedAmt*Qty) " + "FROM S_TimeExpenseLine "
	...
	private void loadDependentInfo()
	{
		...
		if (m_vo.TableName.equals("C_BPartner_Location"))
		{
			...
			if (location_int != null)
				...
				phone_frm = DB.getSQLValueString(null,
						"SELECT ExpressionPhone FROM C_Country c, C_Location l WHERE c.C_Country_ID = l.C_Country_ID AND l.C_location_ID = ?",
						location_int);
	 ...
	 * <p>
	 * It can have an attachment if it has a key column ending with _ID. The key
	 * column is empty, if there is no single identifying key.
	 * 
	...
	public void loadLocks()
	{
		...
		String sql = "SELECT Record_ID " + "FROM AD_Private_Access "
				+ "WHERE AD_User_ID=? AND AD_Table_ID=? AND IsActive='Y' " + "ORDER BY Record_ID";
		try
		{
			if (m_Lock == null)
	...
	public void lock(Properties ctx, int Record_ID, boolean lock)
	{
		...
		log.fine("Lock=" + lock + ", AD_User_ID=" + AD_User_ID + ", AD_Table_ID=" + m_vo.AD_Table_ID + ", Record_ID="
				+ Record_ID);
	...
	/**************************************************************************
	 * Data Status Listener from MTable. - get raw info and add current row
	 * information - update the current row - redistribute (fire) Data Status
	 * event
	 * 
	 * @param e
	 *            event
	...
	public void dataStatusChanged(DataStatusEvent e)
	{
		...
		// log.fine("dataStatusChanged #" + m_vo.TabNo + "- fini",
		// e.toString());
	} // dataStatusChanged
	...
	/**
	 * Navigate absolute - goto Row - (zero based). - does nothing, if in
	 * current row - saves old row if required
	 * 
	 * @param targetRow
	 *            target row
	 * @return current row
	...
	/**
	 * Set current row and load data into fields. If there is no row - load
	 * nulls
	 * 
	 * @param newCurrentRow
	 *            new current row
	...
	private int setCurrentRow(int newCurrentRow, boolean fireEvents)
	{
		...
		for (int i = 0; i < size; i++)
		{
			...
			if (m_currentRow >= 0)
			{
				...
				// The state of the context can be undefined when setCurrentRow
				// is
				// called which can result in unpredictable behaviour. On record
				// insertion, the call to mField.validateValue() happens later
				// in
			...
			{ // no rows - set to a reasonable value - not updateable
				// Object value = null;
				// if (mField.isKey() || mField.isParent() ||
				// mField.getColumnName().equals(m_linkColumnName))
				// value = mField.getDefault();

				// CarlosRuiz - globalqss [ 1881480 ] Navigation problem between
				// tabs
				// the implementation of linking with window context variables
				// is very weak
				// you must be careful when defining a column in a detail tab
				// with a field
				// with the same column name as some of the links of the tabs
				// above
		...
		{
			m_DataStatusEvent = new DataStatusEvent(this, getRowCount(), m_mTable.isInserting(), // changed
	...
	public String setValue(GridField field, Object value)
	{
		...
		//
		return ""; // processFieldChange (field); // here we don't need to call
					// processFieldChange, it was called on
	...
	private void processDependencies(GridField changedField)
	{
		...
		for (int i = 0; i < list.size(); i++)
		{
			...
			// log.trace(log.l5_DData, "Dependent Field", dependentField==null ?
			// "null" : dependentField.getColumnName());
			// if the field has a lookup
			if (dependentField != null && dependentField.getLookup() instanceof MLookup)
			{
				MLookup mLookup = (MLookup) dependentField.getLookup();
				// log.trace(log.l6_Database, "Lookup Validation",
				// mLookup.getValidation());
				// if the lookup is dynamic (i.e. contains this columnName as
				// variable)
	 ...
	 * CalloutSystem. The class needs to comply with the Interface Callout.
	 *
	 * For a limited time, the old notation of Sx_matheod / Ux_menthod is
	 * maintained.
	 *
	 * @param field
	 *            field
	...
	public String processCallout(GridField field)
	{
		...
		while (st.hasMoreTokens()) // for each callout
		{
			...
			if (cmd.toLowerCase().startsWith(MRule.SCRIPT_PREFIX))
			{
				...
				// now add the callout parameters windowNo, tab, field, value,
				// oldValue to the engine
	...
	/**
	 * Get Boolean Value of Field with columnName. If there is no column with
	 * the given name, the context for current window will be checked.
	 * 
	 * @param columnName
	 *            column name
	...
	/*
	 * public boolean isNeedToSaveParent() { if (isDetail()) return
	 * m_parentNeedSave; else return false; }
	 */
	...
	public void switchRows(int from, int to, int sortColumn, boolean ascending)
	{
		...
		{
			lineNoCurrentRow = new Integer(((BigDecimal) m_mTable.getValueAt(from, lineCol)).intValue());
			lineNoNextRow = new Integer(((BigDecimal) m_mTable.getValueAt(to, lineCol)).intValue());
		} else
		{
	...
	private int getParentTabNo()
	{
		...
		// The parent level should be at least one level below. Consider that
		// the level of the
		...
		// The parent is the first currentLevel tab with a level equal to or
		// below
