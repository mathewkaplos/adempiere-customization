 ...
 *         <li>BF [ 1817752 ] MBPartner.getLocations should return only active
 *         one
 * @author Armen Rizal, GOODWILL CONSULT
 *         <LI>BF [ 2041226 ] BP Open Balance should count only Completed
 *         Invoice
...
public class MBPartner extends X_C_BPartner
{
	...
	public static MBPartner get(Properties ctx, String Value)
	{
		...
		MBPartner retValue = new Query(ctx, I_C_BPartner.Table_Name, whereClause, null)
				.setParameters(Value, Env.getAD_Client_ID(ctx)).firstOnly();
		return retValue;
	} // get
	...
	public static MBPartner get(Properties ctx, int C_BPartner_ID)
	{
		...
		MBPartner retValue = new Query(ctx, I_C_BPartner.Table_Name, whereClause, null)
				.setParameters(C_BPartner_ID, Env.getAD_Client_ID(ctx)).firstOnly();
		return retValue;
	} // get
	...
	{
		StringBuffer sb = new StringBuffer("MBPartner[ID=").append(get_ID()).append(",Value=").append(getValue())
				.append(",Name=").append(getName()).append(",Open=").append(getTotalOpenBalance()).append("]");
		return sb.toString();
	} // toString
	...
	/**
	 * Calculate Total Open Balance and SO_CreditUsed. (includes drafted
	 * invoices)
	 */
	public void setTotalOpenBalance()
	{
		...
		String sql = "SELECT "
				...
				+ " AND p.C_Charge_ID IS NULL AND p.DocStatus IN ('CO','CL')),0) " + "FROM C_BPartner bp "
				+ "WHERE C_BPartner_ID=?";
		PreparedStatement pstmt = null;
		try
		{
	...
	public void setSOCreditStatus()
	{
		...
		if (SOCREDITSTATUS_NoCreditCheck.equals(getSOCreditStatus())
				|| SOCREDITSTATUS_CreditStop.equals(getSOCreditStatus()) || Env.ZERO.compareTo(creditLimit) == 0)
			return;
	...
	protected boolean afterSave(boolean newRecord, boolean success)
	{
		...
		if (newRecord && success)
		{
			insertDefaultLoction();
			setRef_ID(getC_BPartner_ID());
			save();
		}
		// mathew...
		if (newRecord)
	...
	private boolean insertDefaultLoction()
	{
		MBPartnerLocation bl = new MBPartnerLocation(getCtx(), 0, get_TrxName());
		bl.setC_BPartner_ID(getC_BPartner_ID());
		bl.setC_Location_ID(1000031);// Default --KENYA
		bl.setName("Bama");
		if (bl.save())
		{
			return true;
	...
	{
		return delete_Accounting("C_BP_Customer_Acct") && delete_Accounting("C_BP_Vendor_Acct")
	...
	/**
	 * @author Mathew 18.05.2017 Get Patient priority EM-Emergency NM-Normal
	 */
	public String getpatient_priority()
	{
		return (String) get_Value("patient_priority");
	}

	public void setPatientPriority(String pp)
	{
		set_Value("patient_priority", pp);
	}
	...
	{
		return (Timestamp) get_Value("dob");
	}

	public boolean getIsBooked()
	{
		Object oo = get_Value("is_booked");
		if (oo != null)
		{
	...
	{
		set_Value("is_booked", Boolean.valueOf(isBooked));
	}
	 ...
	 * 
	 */
	public void setNextLocation(String next_location)
	{
		set_Value("next_location", next_location);
	}

	public String getNextLocation()
	{
		return get_ValueAsString("next_location");
	}
	...
	{
		set_Value("current_location", current_location);
	}

	public String getCurrentLocation()
	{
		return get_ValueAsString("current_location");
	}
	// old_patient_no
	...
	{
		set_Value("old_patient_no", OldPatientNo);
	}
	...
	{
		return (String) get_Value("old_patient_no");
	}

	/**
	 * 
	 * @param old_patient_system_id
	 */
	public void setOldPatientSystemID(int old_patient_system_id)
	{
		set_Value("old_patient_system_id", old_patient_system_id);
	}

	public int getOldPatientSystemID()
	{
		return get_ValueAsInt("old_patient_system_id");
	}
	...
	{
		return (Timestamp) get_Value("dor");
	}
	...
	{
		return get_ValueAsString("id_no");
	}
	...
	{
		return get_ValueAsString("notes");
	}

	// tel_no
	public void setTelNo(String tel_no)
	{
		set_Value("tel_no", tel_no);
	}

	public String getTelNo()
	{
		return (String) get_Value("tel_no");
	}

	// nhif_no
	public void setNHIFNo(String nhif_no)
	{
		set_Value("nhif_no", nhif_no);
	}

	public String getNHIFNo()
	{
		return (String) get_Value("nhif_no");
	}

	// contribute_to_nhif
	public boolean getContributesToNHIF()
	{
		Object oo = get_Value("contribute_to_nhif");
		if (oo != null)
		{
			if (oo instanceof Boolean)
				return ((Boolean) oo).booleanValue();
	...
	public void setContributesToNHIF(boolean contributes)
	{
		set_Value("contribute_to_nhif", Boolean.valueOf(contributes));
	}
	...
	{
		set_Value("marital_status", marital_status);
	}

	// Reference ID
	public void setRef_ID(int ref_id)
	{
		set_Value("ref_id", ref_id);
	}
	...
	{
		set_Value("documentno", docNo);
	}

	// inpatient_no
	public int getInpatientNo()
	{
		return get_ValueAsInt("inpatient_no");
	}

	public void setInpatientNo(int inpatientNo)
	{
		set_Value("inpatient_no", inpatientNo);
	}

	// is_self_employed
	public boolean getIsSelfEmployed()
	{
		Object oo = get_Value("is_self_employed");
		if (oo != null)
		{
			if (oo instanceof Boolean)
				return ((Boolean) oo).booleanValue();
	...
	{
		set_Value("is_self_employed", Boolean.valueOf(isSelfEmployed));
	}

	// photo_attached
	public boolean isPhotoAttached()
	{
		Object oo = get_Value("photo_attached");
		if (oo != null)
		{
			if (oo instanceof Boolean)
				return ((Boolean) oo).booleanValue();
	...
	public void setIsPhotoAttached(boolean attached)
	{
		set_Value("photo_attached", Boolean.valueOf(attached));
	}

	// hms_insuredco_id
	public int getInsuredCompany_ID()
	{
		return get_ValueAsInt("hms_insuredco_id");
	}

	public void setInsuredCompany_ID(int companyID)
	{
		set_Value("hms_insuredco_id", companyID);
	}

	// age
	public String getAge()
	{
		String sql = "SELECT age(bp.dob)  FROM adempiere.C_BPartner bp WHERE bp.C_BPartner_ID =?";
	...
	public String getPeriodSinceLastVist()
	{
		String sql = "(SELECT   CASE WHEN z.a>(age(now()::date-age(now()::date))) THEN z.a::text"
				+ " ELSE '0 days'::text END FROM ( SELECT age(doc.created::date) AS a"
				+ " FROM adempiere.hms_treatment_doc doc  WHERE doc.hms_treatment_doc_id = ( select max(d.hms_treatment_doc_id)  "
				+ "   FROM adempiere.hms_treatment_doc d where d.c_bpartner_id =?) ) AS z)";
		return DB.getSQLValueString(get_TrxName(), sql, getC_BPartner_ID());
	}

	public static boolean exists(int C_BPartner_ID)
	{
		boolean exist = false;
	...
	{
		return get_ValueAsString("member_number");
	}

	public int getInsuranceCompany_ID()
	{
		int value = 0;
		final int hms_insuredco_id = this.getInsuredCompany_ID();
		if (hms_insuredco_id > 0)
		{
			...
			try
			{
				final String sql = "SELECT hms_insco_ID  FROM adempiere.hms_insuredco  WHERE hms_insuredco_id ="
						+ hms_insuredco_id;
				stm = (PreparedStatement) DB.prepareStatement(sql, (String) null);
				rs = stm.executeQuery();
				while (rs.next())
				{
					final int x = rs.getInt(1);
					if (x > 0)
					{
						value = x;
					}
	...
	public BigDecimal getHMSBalance()
	{
		BigDecimal balance = Env.ZERO;
		final String sql = "SELECT  SUM(COALESCE(bill.balance,0)) AS balance FROM  adempiere.hms_billing bill WHERE bill.invoiced ='N' AND bill.c_bpartner_id =  "
	...
	public boolean bookedRecently()
	{
		...
		final long REBOOKING_INTERVAL = HmsSetup.getSetup().getrebooking_interval() * 60 * 60 * 1000;
