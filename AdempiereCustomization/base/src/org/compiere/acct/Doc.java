...
public abstract class Doc
{
	...
	/**************************************************************************
	 * Document Types -------------- C_DocType.DocBaseType & AD_Reference_ID=183
	...
	public static Doc get(MAcctSchema[] ass, int AD_Table_ID, ResultSet rs, String trxName) throws AdempiereUserError
	{
		...
		/*
		 * Classname of the Doc class follows this convention: if the prefix
		 * (letters before the first underscore _) is 1 character, then the
		 * class is Doc_TableWithoutPrefixWithoutUnderscores otherwise
		 * Doc_WholeTableWithoutUnderscores i.e. following this query SELECT
		 * t.ad_table_id, tablename, CASE WHEN instr(tablename, '_') = 2 THEN
		 * 'Doc_' || substr(tablename, 3) WHEN instr(tablename, '_') > 2 THEN
		 * 'Doc_' || ELSE '' REPLACE ( tablename, '_', '' ) END AS classname
		 * FROM ad_table t, ad_column C WHERE t.ad_table_id = C.ad_table_id AND
		 * C.columnname = 'Posted' AND isview = 'N' ORDER BY 1 This is: 224
		 * GL_Journal Doc_GLJournal 259 C_Order Doc_Order 318 C_Invoice
		 * Doc_Invoice 319 M_InOut Doc_InOut 321 M_Inventory Doc_Inventory 323
		 * M_Movement Doc_Movement 325 M_Production Doc_Production 335 C_Payment
		 * Doc_Payment 392 C_BankStatement Doc_BankStatement 407 C_Cash Doc_Cash
		 * 472 M_MatchInv Doc_MatchInv 473 M_MatchPO Doc_MatchPO 623
		 * C_ProjectIssue Doc_ProjectIssue 702 M_Requisition Doc_Requisition 735
		 * C_AllocationHdr Doc_AllocationHdr 53027 PP_Order Doc_PPOrder 53035
		 * PP_Cost_Collector Doc_PPCostCollector 53037 DD_Order Doc_DDOrder
		...
		try
		{
			...
			Constructor<?> cnstr = cClass
					.getConstructor(new Class[] { MAcctSchema[].class, ResultSet.class, String.class });
	...
	/*
	 * If the transaction must be managed locally (false if it's managed
	 ...
	 * 
	 * @param force
	 *            if true ignore that locked
	 * @param repost
	 *            if true ignore that already posted
	...
	public final String post(boolean force, boolean repost)
	{
		...
		{
			String error = "AD_Client_ID Conflict - Document=" + p_po.getAD_Client_ID() + ", AcctSchema="
					+ m_ass[0].getAD_Client_ID();
			log.severe(error);
			return error;
		}
		...
		if (repost)
		{
			if (isPosted() && !isPeriodOpen()) // already posted - don't delete
												// if period closed
			{
		...
		if (!p_Status.equals(STATUS_Posted))
		{
			...
			Text.append(" - ").append(cn.substring(cn.lastIndexOf('.'))).append(" (").append(getDocumentType())
					.append(" - DocumentNo=").append(getDocumentNo()).append(", DateAcct=")
					.append(getDateAcct().toString().substring(0, 10)).append(", Amount=").append(getAmount())
					.append(", Sta=").append(p_Status).append(" - PeriodOpen=").append(isPeriodOpen())
	...
	{
		StringBuffer sql = new StringBuffer("DELETE Fact_Acct WHERE AD_Table_ID=").append(get_Table_ID())
	...
	/**
	 * Calculate Period from DateAcct. m_C_Period_ID is set to -1 of not open to
	 * 0 if not found
	 */
	...
	public int getValidCombination_ID(int AcctType, MAcctSchema as)
	{
		...
		else if (AcctType == ACCTTYPE_InvDifferences)
		{
			...
			// "SELECT W_Inventory_Acct, W_Revaluation_Acct,
			// W_InvActualAdjust_Acct FROM M_Warehouse_Acct WHERE
	...
	private final boolean save(String trxName)
	{
		...
		StringBuffer sql = new StringBuffer("UPDATE ");
		sql.append(get_TableName()).append(" SET Posted='").append(p_Status).append("',Processing='N' ")
				.append("WHERE ").append(get_TableName()).append("_ID=").append(p_po.get_ID());
	...
	private static void fillDocumentsTableArrays()
	{
		...
		{
			String sql = "SELECT t.AD_Table_ID, t.TableName " + "FROM AD_Table t, AD_Column c "
