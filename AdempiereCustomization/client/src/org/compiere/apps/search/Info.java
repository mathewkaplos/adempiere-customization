 ...
 */
public abstract class Info extends CDialog implements ListSelectionListener, PropertyChangeListener
{
	/**
	 * 
	 */
	 ...
	 *            window no
	 * @param tableName
	 *            table name of the search
	 * @param keyColumn
	 *            key column of the search
	 * @param value
	 *            query value to find, exclusive of record_id
	 * @param multiSelection
	 *            allow to select more than one row
	 * @param whereClause
	 *            fully qualified where clause for the search
	 * @return special or general Info Window
	 */
	@Deprecated
	public static Info create(Frame frame, boolean modal, int WindowNo, String tableName, String keyColumn,
			String value, boolean multiSelection, String whereClause)
	{
		return create(frame, modal, WindowNo, tableName, keyColumn, 0, value, multiSelection, true, whereClause);
	}
	 ...
	 */
	public static Info create(Frame frame, boolean modal, int windowNo, String tableName, String keyColumn,
			int record_id, String value, boolean multiSelection, boolean saveResult, String whereClause)
	{
		Info info = null;
      
		if (tableName.equals("C_BPartner"))
			info = new InfoBPartner(frame, modal, windowNo, record_id, value,
					!Env.getContext(Env.getCtx(), "IsSOTrx").equals("N"), false, multiSelection, saveResult,
					whereClause);
		else if (tableName.equals("M_Product"))
			info = new InfoProduct(frame, modal, windowNo, 0, 0, record_id, value, multiSelection, saveResult,
					whereClause);
		else if (tableName.equals("C_Invoice"))
			info = new InfoInvoice(frame, modal, windowNo, record_id, value, multiSelection, saveResult, whereClause);
		else if (tableName.equals("A_Asset"))
			info = new InfoAsset(frame, modal, windowNo, record_id, value, multiSelection, saveResult, whereClause);
		else if (tableName.equals("C_Order"))
			info = new InfoOrder(frame, modal, windowNo, record_id, value, multiSelection, saveResult, whereClause);
		else if (tableName.equals("M_InOut"))
			info = new InfoInOut(frame, modal, windowNo, record_id, value, multiSelection, saveResult, whereClause);
		else if (tableName.equals("C_Payment"))
			info = new InfoPayment(frame, modal, windowNo, record_id, value, multiSelection, saveResult, whereClause);
		else if (tableName.equals("C_CashLine"))
			info = new InfoCashLine(frame, modal, windowNo, record_id, value, multiSelection, saveResult, whereClause);
		else if (tableName.equals("S_ResourceAssigment"))
			info = new InfoAssignment(frame, modal, windowNo, record_id, value, multiSelection, saveResult,
					whereClause);
		else if (tableName.equals("hms_test"))
			info = new InfoMOM(frame, modal, windowNo, record_id, value, true, saveResult,
					whereClause);
		//hms_specimen_requests
		else if (tableName.equals("hms_specimen_requests"))
			info = new InfoResults(frame, modal, windowNo, record_id, value, false, saveResult,
					whereClause);
		//hms_diagnosis
		else if (tableName.equals("hms_diagnosis"))
			info = new InfoDiagnosis(frame, modal, windowNo, record_id, value, false, saveResult,
					whereClause);
		else
			info = new InfoGeneral(frame, modal, windowNo, record_id, value, tableName, keyColumn, multiSelection,
	...
	public static void showProduct(Frame frame, int WindowNo)
	{
		Info info = new InfoProduct(frame, false, WindowNo,
				...
				Env.getContextAsInt(Env.getCtx(), WindowNo, "M_PriceList_ID"), 0, // Record
																					// ID
				"", // value
				false, false, "");
		AEnv.showCenterWindow(frame, info);
	 ...
	 *            window no
	 */
	public static void showProduct2(Frame frame, int WindowNo)
	{
		Info info = new InfoMOM(frame, false, WindowNo, 0, // Record ID
				"", // value
				false, false, "");
		AEnv.showCenterWindow(frame, info);
	}
	...
	{
		Info info = new InfoInvoice(frame, false, WindowNo, 0, value, false, false, "");
		AEnv.showCenterWindow(frame, info);
	} // showInvoice
	...
	{
		Info info = new InfoInOut(frame, false, WindowNo, 0, value, false, false, "");
		AEnv.showCenterWindow(frame, info);
	} // showInOut
	...
	{
		Info info = new InfoPayment(frame, false, WindowNo, 0, value, false, false, "");
		AEnv.showCenterWindow(frame, info);
	} // showPayment
	...
	{
		Info info = new InfoCashLine(frame, false, WindowNo, 0, value, false, false, "");
		AEnv.showCenterWindow(frame, info);
	} // showCashLine
	...
	{
		Info info = new InfoAssignment(frame, false, WindowNo, 0, value, false, false, "");
		AEnv.showCenterWindow(frame, info);
	} // showAssignment
	 ...
	 *            key column name
	 * @param saveResults
	 *            flag if the results will be saved in context
	 * @param multiSelection
	 *            multiple selection
	 * @param whereClause
	 *            where clause
	 */
	protected Info(Frame frame, boolean modal, int WindowNo, String tableName, String keyColumn, boolean multiSelection,
			boolean saveResults, String whereClause)
	{
		...
		{
			// Activated from a window - find the active tab to limit the
			// context
	...
	/**
	 * Reset the record ID - false on load, reset by any action that reruns the
	 * query
	 */
	...
	protected void jbInit() throws Exception
	{
		...
		addonPanel.add(p_detailTaskPane, BorderLayout.CENTER); // Allow auto
																// resizing of
																// the panel
		...
		confirmPanel.setOKVisible(p_saveResults); // Only show the OK button if
													// we intend to save
		//
		...
		checkAutoQuery.setSelected(
				MSysConfig.getValue(SYSCONFIG_INFO_AUTO_QUERY, "Y", Env.getAD_Client_ID(Env.getCtx())).equals("Y"));
		checkAutoQuery.addActionListener(this);
		CPanel leftButtons = (CPanel) confirmPanel.getComponent(1); // Index 0
																	// is
																	// OK-Cancel
																	// on the
																	// right
		leftButtons.add(checkAutoQuery, 0); // Add the check box on the very
											// left before the refresh icon
		//
		popup.add(zoomMenu);
		...
		((ListSelectionModel) p_table.getSelectionModel()).addListSelectionListener(this); // To
																							// enable
																							// buttons
		...
		//
		m_parentPanel = p_criteriaGrid; // Set default location where focus will
										// go. See property change listener.
		//
		enableButtons();
	} // jbInit
	...
	protected ArrayList<Integer> getSelectedRowKeys()
	{
		...
		if (p_table.isMultiSelection())
		{
			...
			{
				// If this is the Totals row (last row), we need to skip it -
				// teo_sarca [ 2860556 ]
	...
	protected boolean setSelectedRow(int record_id)
	{
		...
		if (p_table.isMultiSelection() && p_table.isDefaultSelected()) // Select
																		// all
																		// by
																		// default
		{
		...
		if (selectedKey != null && selectedKey.intValue() == record_id) // We're
																		// already
																		// there
			return true;

		// Nothing or the wrong row selected - try to find the record in the
		// table
		int rows = p_table.getRowCount();
	...
	/**
	 * Is cancelled? - if pressed Cancel = true - if pressed OK or window closed
	 * = false
	 * 
	...
	public static String getSQLText(String s)
	{
		...
		// It can be "%*" for first-only, "*%" for last-only, "%*%" for both or
		// "*" for none.
		// The pattern string must start and/or end with a "%" symbol. The "*"
		// symbol can be any string.
		// The default is last-only. "%" or "%%" are valid and will be
		// interpreted as both.
		String wildCardPattern = MSysConfig.getValue(SYSCONFIG_INFO_AUTO_WILDCARD, "*%",
	...
	public void propertyChange(PropertyChangeEvent e)
	{
		...
		{
			// One query has been performed. From now on, ignore the record_id
			// and use the search criteria.
			...
			if (!setSelectedRow(p_selectedRecordKey))
			{
				...
				noRecordSelected();
			} else // Found and selected the same record or selected the first
					// record
			{
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		if (e.getSource() != null)
		{
			...
			{
				// The enter key is mapped to the Ok button which will close the
				// dialog.
				// Don't let this happen if there are outstanding changes to any
				// of the
			...
			} else if (source instanceof VComboBox)
			{
				if (((VComboBox) source).getParent() instanceof VLookup)
				{
					...
					// Respond only to the Enter key which causes
					// "comboBoxEdited"
			...
			} else if (source instanceof CTextField)
			{
				...
				if (tf.getParent() instanceof VLookup)
				{
					...
					// instead.
					return;
				} else if (tf.hasChanged() || hasOutstandingChanges()) // The
																		// change
																		// may
																		// have
																		// come
				...
				{
					// Special case where text fields don't change but cause an
					// event
					// Interpret this as a click of the OK button and close
					// EXCEPT
		...
		// Refresh if the autoquery feature is selected or the refresh button is
		// clicked.
	...
	/**
	 * Prepare for Requery of the table. If dynamic changes are pending, prepare
	 * the table.
	 */
	...
	{
		// ValueChanged triggered by count can be ignored. Event is fired in the
		// info worker run()
		// when the table is cleared.
		if (m_ignoreEvents || p_table.getRowCount() == 0)
			return;

		// Mouse events cause duplicate firings of the valueChanged event. Trap
		// the duplicate.
		if (m_lse.equals(lse))
			return;
		m_lse = lse;
	...
	/**
	 * Enable OK, History, Zoom if row/s selected --- Changes: Changed the logic
	 * for accommodating multiple selection
	 * 
	 * @author ashley
	 */
	 ...
	 * 
	 * @param o
	 *            - one of a VLookup, VAttributeInstance
	 * @return
	 */
	public boolean isValidVObject(Object o)
	{
		if (o != null)
		{
			try
			{
				...
				{
					return (((VPAttribute) o).getValue() != null
							&& ((Integer) ((VPAttribute) o).getValue()).intValue() != 0);
				}
	...
	{
		// If we want to save results, double click in a row will cause save and
		// exit
		// The miniTable class will trap double clicks when multiselection is
		// enabled
		// and the configuration is set to toggle the selection on double clicks
		if (p_saveResults && e.getID() == MouseEvent.MOUSE_CLICKED && e.getClickCount() > 1
				&& p_table.getSelectedRow() != -1)
		{
			dispose(p_saveResults);
		}
	...
	{
		// Has a popup trigger occurred? Need to test both pressed and released
		// for cross-platform
		// compatibility
		if (e.isPopupTrigger())
		{
	...
	class Worker extends Thread
	{
		...
		public void run()
		{
				...
				while (m_rs.next())
				{
					...
					for (int col = 0; col < p_layout.length; col++)
					{
						...
						if (c == IDColumn.class)
						{
							...
							if (p_table.isMultiSelection()) // Multiple rows can
															// be selected
							{
								if (p_table.isDefaultSelected()) // Select all
																	// by
																	// default
								{
									p_table.changeSelection(row, 0, false, (row != 0)); // Anchor
																						// at
																						// the
																						// first
																						// row,
						 ...
						 * p_layout[col].getColHeader() + " data=" +
						 * data.toString() + " " + data.getClass().getName() +
						 * " * " + p_table.getCellRenderer(row, col)); else
						 * log.fine( "r=" + row + ", c=" + col + " " +
						 * p_layout[col].getColHeader() + " data=" + null +
						 * " String * " + p_table.getCellRenderer(row, col));
						 */

						p_table.setValueAt(data, p_table.convertRowIndexToView(row),
								p_table.convertColumnIndexToView(col));
					}
				}
			...
			{
				if (!p_table.isMultiSelection())
					p_table.getSelectionModel().setSelectionInterval(0, 0); // Select
																			// the
																			// first
																			// row
																			// only.
			...
			p_table.firePropertyChange("p_table_update", 0, 1); // Inform the
																// subclass of
																// the change
		} // run
		...
		/**
		 * Interrupt this thread - cancel the query if still in execution Carlos
		 * Ruiz - globalqss - [2826660] - Info product performance BIG problem
		 */
	 ...
	 * in a query? Override with specific tests.
	 * 
	...
	/**
	 * Generic init call invoked by the event handler to reset the criteria
	 * panel. Used to call class specific initInfo function with reset
	 * parameters.
	 */
	 ...
	 * 
	 * @param record_id
	 *            The ID of the record to display or zero
	 * @param value
	 *            value
	 */
