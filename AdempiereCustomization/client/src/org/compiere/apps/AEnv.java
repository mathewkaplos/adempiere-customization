...
public final class AEnv
{
	...
	/**
	 * Position in center of the parent window. (pack, set location and set
	 * visibility)
	 * 
	 * @param parent
	 *            Parent Window
	...
	public static void positionCenterWindow(Window parent, Window window)
	{
		...
		if (y + wSize.height > sSize.height)
			y = sSize.height - wSize.height;
		//
		// System.out.println("Position: x=" + x + " y=" + y + " w=" +
		// wSize.getWidth() + " h=" + wSize.getHeight()
		// + " - Parent loc x=" + pLoc.x + " y=" + y + " w=" + pSize.getWidth()
		// + " h=" + pSize.getHeight());
	...
	/**
	 * Create Menu Title (translate it and set Mnemonics). Based on MS notation
	 * of &Help => H is Mnemonics
	 *
	 * @param AD_Message
	 *            message
	 * @return JMenu
	 ...
	 *            action command
	 * @param iconName
	 *            optional name of the icon, defaults to action if null
	 * @param ks
	 *            optional key stroke
	 * @param menu
	 *            menu to add menu item to
	 * @param al
	 *            action listener to register
	 * @return MenuItem
	 */
	...
	/**
	 * Perform action command for common menu items. Created in
	 * AMenu.createMenu(), APanel.createMenu(), FormFrame.createMenu()
	 * 
	 * @param actionCommand
	...
	public static boolean actionPerformed(String actionCommand, int WindowNo, Container c)
	{
		...
		else if (actionCommand.equals("InfoMRP") && AEnv.canAccessInfo("MRP"))
		{
			...
			AMenuStartItem form = new AMenuStartItem(m_menu_id, true, Msg.translate(Env.getCtx(), "MRP Info"), menu); // async
																														// load
			form.start();
		} else if (actionCommand.equals("InfoCRP") && AEnv.canAccessInfo("CRP"))
		{
			...
			AMenu menu = AEnv.getAMenu(frame);
			AMenuStartItem form = new AMenuStartItem(m_menu_id, true, Msg.translate(Env.getCtx(), "CRP Info"), menu); // async
																														// load
			form.start();
		...
		} else if (actionCommand.equals("InfoMOM"))
		{
			org.compiere.apps.search.Info.showProduct2(Env.getFrame(c), WindowNo);
			// new org.compiere.apps.search.InfoMOM (Env.getFrame(c), null,
			// false);
		}
	...
	/**
	 * Set Text and Mnemonic for Button. Create Mnemonics of text containing
	 * '&'. Based on MS notation of &Help => H is Mnemonics
	 * 
	 * @param b
	 *            The button
	 * @param text
	 *            The text with optional Mnemonics
	 */
	...
	public static boolean isWorkflowProcess()
	{
		if (s_workflow == null)
		{
			...
			{
				s_workflow_Window_ID = DB.getSQLValue(null, "SELECT AD_Window_ID FROM AD_Table WHERE AD_Table_ID=?",
						AD_Table_ID);
	 ...
	 *            window
	 * @param AD_Table_ID
	 *            Table ID of Document
	 * @param AD_Client_ID
	 *            Client ID of Document
	 * @param Record_ID
